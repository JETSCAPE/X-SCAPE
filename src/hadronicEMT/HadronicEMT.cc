...
void HadronicEMT::InitTask() {
  ...
  sigma_transverse_ = JetScapeXML::Instance()->GetElementDouble(
      {"HadronicEMT", "sigma_transverse"});
  sigma_longitudinal_ = JetScapeXML::Instance()->GetElementDouble(
      {"HadronicEMT", "sigma_longitudinal"});
  smearing_covariant_ = JetScapeXML::Instance()->GetElementInt(
      {"HadronicEMT", "smearing_covariant"});
...
void HadronicEMT::ComputeEnergyDensityAndFlowVelocity(
  ...
  std::array<std::array<double, 4>, 4> Tmualpha = {
  ...
  int status =
      gsl_eigen_nonsymmv(Tmualpha_gsl_, eigenvalues_, eigenvectors_, w_);
...
void HadronicEMT::ParseEigenSystem(gsl_matrix_complex *eigen_vectors,
  ...
  for (int i = 0; i < 4; i++) {
    // get the real and imaginary part of the eigenvalue and store them in
    // double
    ...
    if (eval_real_part > 0.0 &&
        (fabs(eval_imag_part / eval_real_part) < imag_tolerance)) {
      // take the corresponding eigenvector to check it
      gsl_vector_complex_view evec_i =
          gsl_matrix_complex_column(eigen_vectors, i);
      ...
      for (int j = 0; j < 4; j++) {
        ...
        const double evec_real =
            GSL_REAL(gsl_matrix_complex_get(eigen_vectors, j, i));
        const double evec_imag =
            GSL_IMAG(gsl_matrix_complex_get(eigen_vectors, j, i));
 ...
 * `Tmn_requested_point_`.
 */
std::array<std::array<double, 4>, 4> HadronicEMT::GetEnergyMomentumTensor()
    const {
...
void HadronicEMT::AddParticleToEnergyMomentumTensor(Hadron &hadron,
  ...
  if (e_raw <= eps) {
    VERBOSE(8) << "HadronicEMT: Skipping particle with very low or negative "
                  "energy: e = "
               << e_raw;
    return;
  }
 ...
 */
double HadronicEMT::smearing_kernel_gaussian(const double x_diff,
                                             const double y_diff,
                                             const double z_diff) {
...
/**
 * @brief Calculates and fills the bulk medium properties at a given space-time
 * point.
 *
 ...
 * @param bulk_info_ptr A unique pointer to a `BulkMediaInfo` object to store
 * the computed results.
 * @param h_list A vector of `Hadron` objects representing the hadrons in the
 * vicinity of the point.
 *
 * @details
 * - Resets the energy-momentum tensor to zero using
 * `ResetEnergyMomentumTensor()`.
 * - Loops over all hadrons in `h_list` to compute their contribution to the
 * energy-momentum tensor using either a covariant or Gaussian smearing kernel.
 * - Contributions are ignored for hadrons beyond a specified transverse or
 * longitudinal skip distance.
 * - After computing the contributions, the energy-momentum tensor is stored in
 * `bulk_info_ptr`.
 * - If the energy density is below a threshold (`rounding_error`), the bulk
 * properties are set to zero.
 * - Otherwise, the energy density and flow velocity are computed using
 * `ComputeEnergyDensityAndFlowVelocity`.
 * - The temperature and entropy density are obtained using the equation of
 * state (EOS) functions `get_T` and `get_s`.
 *
 * @note The covariant smearing kernel is used if `smearing_covariant_` is set;
 * otherwise, a Gaussian smearing kernel is applied.
 *
 * @warning Assumes that all hadrons are at the same time step during the
...
void HadronicEMT::GetBulkInfo(Jetscape::real t, Jetscape::real x,
  ...
  for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 4; ++j) {
      bulk_info_ptr->tmn[i][j] =
          static_cast<Jetscape::real>(Tmn_requested_point_[i][j]);
    }
  }
  ...
  ComputeEnergyDensityAndFlowVelocity(Tmn_requested_point_, e_requested_point_,
                                      umu_requested_point_);
...
/**
 * @brief Determines which hadrons should be fluidized based on the local
 * temperature of the medium.
 *
 * This function evaluates a 3D grid of spatial points and computes the bulk
 * medium properties (e.g., temperature) at each point. If the temperature at a
 * grid point exceeds the critical temperature \( T_{\text{critical}} \),
 * hadrons near that point are flagged for fluidization.
 *
 * @param T_critical The critical temperature above which hadrons are flagged
 * for fluidization.
 * @param current_hadrons A vector of `Hadron` objects representing the current
 * hadron population.
 * @return A vector of booleans, where each entry corresponds to a hadron in
 * `current_hadrons`. `true` indicates that the corresponding hadron should be
 * fluidized.
 *
 * @details
 * - The function initializes a boolean vector, `fluidize_hadrons`, with the
 * same size as `current_hadrons`, setting all entries to `false`.
 * - Loops over a 3D grid of spatial points with ranges defined by `Nx_`, `Ny_`,
 * and `Nz_` and spacing determined by `dx_`, `dy_`, and `dz_`.
 * - At each grid point, `GetBulkInfo` is called to compute the bulk medium
 * properties.
 * - If the temperature at a grid point exceeds `T_critical`, all hadrons within
 * a specified smearing range around that point are flagged for fluidization.
 * - The smearing ranges are determined by `sigma_transverse_` and
 * `sigma_longitudinal_`, with adjustments for covariant smearing if
 * `smearing_covariant_` is enabled.
 *
 * @note
 * - Covariant smearing adjusts the longitudinal smearing range to match the
 * transverse range.
 * - Assumes all hadrons and grid points are evaluated at the same time step
 * (time coordinate fixed at \( t = 0 \)).
 *
 * @warning
 * - The function loops over all grid points and hadrons, which may be
 * computationally expensive for large grids or hadron populations.
 */
std::vector<bool> HadronicEMT::DetermineHadronsForFluidization(
...
void HadronicEMT::read_EOS_from_file(const std::string &filename) {
  ...
  if (!file.is_open()) {
    throw std::runtime_error("HadronicEMT: Could not open EOS file " +
                             filename);
  }
...
/**
 * @brief Interpolates the equation of state (EOS) data for a given energy
 * density.
 *
 ...
 *   values.
 */
double HadronicEMT::interpolate_1D_EOS(double e,
  ...
  int idx_e = std::clamp(static_cast<int>((e - e_lower_bound_) / e_spacing_), 0,
                         e_length_ - 2);
  ...
  const double frac_e =
      (e - (idx_e * e_spacing_ + e_lower_bound_)) / e_spacing_;
...
/**
 * @brief Computes the Cartesian time coordinate from the proper time and
 * pseudorapidity.
 *
...
/**
 * @brief Computes the Cartesian coordinate \( z \) from the proper time and
 * pseudorapidity.
 *
 ...
 *
 * This function calculates the proper time component \( \tau \) of a four
 * vector based on the energy \( p^0 \), momentum \( p^3 \), and pseudorapidity
 * \( \eta \).
 *
 ...
 *
 * This function calculates the pseudorapidity component \( \eta \) of a four
 * vector based on the energy \( p^0 \), momentum \( p^3 \), and proper time \(
 * \tau \).
 *
 * @param p0 The energy component of the four vector.
 * @param p3 The momentum component of the four vector.
