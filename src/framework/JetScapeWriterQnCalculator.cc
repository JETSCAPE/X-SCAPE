...
RegisterJetScapeModule<JetScapeWriterQnVectorStream<ofstream>>
    JetScapeWriterQnVectorStream<ofstream>::regQnVector(
        "JetScapeWriterQnVectorAscii");
template <>
RegisterJetScapeModule<JetScapeWriterQnVectorStream<ogzstream>>
    JetScapeWriterQnVectorStream<ogzstream>::regQnVectorGZ(
        "JetScapeWriterQnVectorAsciiGZ");
...
void JetScapeWriterQnVectorStream<T>::WriteEvent() {
  ...
  for (int id = 0; id < pdglist.size(); id++) {
    ...
    std::shared_ptr<Qvector> Qvec_tem_ptr =
        std::make_shared<Qvector>(pTmin_, pTmax_, npT_, rapmin_, rapmax_, nrap_,
    ...
    for (int ipt = 0; ipt < Qvec_tem_ptr->get_npt(); ipt++) {
      for (int iy = 0; iy < Qvec_tem_ptr->get_ny(); iy++) {
        ...
        double total_dN_mean =
            Qvec_tem_ptr->get_value(ipt, iy, 5) / oversamplenevent;
        ...
        if (total_dN_mean > 0.0) {
          ...
          mean_pT_err = sqrt(Qvec_tem_ptr->get_value(ipt, iy, 1) / total_dN -
                             mean_pT * mean_pT) /
                        sqrt(total_dN);
          mean_y = Qvec_tem_ptr->get_value(ipt, iy, 2) / total_dN;
          mean_y_err = sqrt(Qvec_tem_ptr->get_value(ipt, iy, 3) / total_dN -
                            mean_y * mean_y) /
                       sqrt(total_dN);
        } else {
        ...
        output_file << mean_pT << " " << mean_pT_err << " " << mean_y << " "
                    << mean_y_err << " " << total_ET_mean << " "
                    << total_dN_mean / (dy * dpt) << " "
                    << total_dN_mean_err / (dy * dpt) << " ";
        for (int iorder = 1; iorder < norder_; iorder++) {
          ...
          if (total_dN_mean > 0.0) {
            Qn_real_mean =
                Qvec_tem_ptr->get_value(ipt, iy, 4 * iorder + 3) / total_dN;
            Qn_imag_mean =
                Qvec_tem_ptr->get_value(ipt, iy, 4 * iorder + 4) / total_dN;
            Qn_real_err =
                sqrt(Qvec_tem_ptr->get_value(ipt, iy, 4 * iorder + 5) /
                         total_dN -
                     Qn_real_mean * Qn_real_mean) /
                sqrt(total_dN);
            if (std::isnan(Qn_real_err))
              Qn_real_err = 0.0;
            Qn_imag_err =
                sqrt(Qvec_tem_ptr->get_value(ipt, iy, 4 * iorder + 6) /
                         total_dN -
                     Qn_imag_mean * Qn_imag_mean) /
                sqrt(total_dN);
            if (std::isnan(Qn_imag_err))
              Qn_imag_err = 0.0;
          }
...
template <class T>
void JetScapeWriterQnVectorStream<T>::Close() {
