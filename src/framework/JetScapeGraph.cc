...
template <class T>
void JetScapeGraph<T>::InsertParticle(edge e, shared_ptr<Vertex> v,
                                      shared_ptr<T> p) {
  node nS = e.source();
  node nE = e.target();
...
template <class T>
void JetScapeGraph<T>::InsertParticleAfter(edge e, shared_ptr<Vertex> v,
                                           shared_ptr<T> p) {
  node nS = e.source();
  node nE = e.target();
...
vector<fjcore::PseudoJet> JetScapeGraph<T>::GetFinalParticlesForFastJet() {
  vector<shared_ptr<T>> mP =
      GetFinalParticles();  // to ensure that pFinal is filled
...
template <class T>
void JetScapeGraph<T>::GetBfsSortedListOfNodesAndEdges(vector<node> &nl,
                                                       vector<edge> &el) {
  bfs search;
  search.reset();
  search.scan_whole_graph(true);
...
void JetScapeGraph<T>::SaveAsGV(string fName) {
  ...
  for (eIt = edges_begin(), eEnd = edges_end(); eIt != eEnd; ++eIt) {
    ...
    gv << to_string(eIt->source().id()) + "->" + to_string(eIt->target().id())
       << " " << label << stream1.str() << stream.str() << label2 << endl;
  }
