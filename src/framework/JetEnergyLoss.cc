...
JetEnergyLoss::JetEnergyLoss(const JetEnergyLoss &j) {
  ...
  AddModuleClock(
      j.GetModuleClock());  // JP: Check if memory leak ... should not due to
                            // shared_ptr usage, but confirm ...
...
void JetEnergyLoss::InitTask() {
  ...
  if (GetActive())
    JSINFO << "Eloss shower with deltaT = " << deltaT
           << " startT = " << GetStartT() << " and maxT = " << maxT;
  else
...
void JetEnergyLoss::DoShower() {
  ...
  JSDEBUG << "Run with starT = " << GetStartT() << " , maxT = " << GetMaxT()
          << " , GetDeltaT = " << GetDeltaT();
...
void JetEnergyLoss::ExecuteTask() {
  ...
  if (GetShowerInitiatingParton() && !useShower) {
    ...
    if (!GetPartonShowerGenerator()) {
      // VERBOSE(8)<<"Use Default DoShower() to do Parton shower stored in
      // PartonShower Graph class";
      JSDEBUG << "--> Use Default DoShower() from JetEnergyLoss to do Parton "
                 "shower stored in PartonShower Graph class";

      // Shower handled in this class ...
      DoShower();
    } else if (GetPartonShowerGenerator()) {
      JSDEBUG << "--> Use DoShower() provided from PSG to do Parton shower "
                 "stored in PartonShower Graph class";

      GetPartonShowerGenerator()->DoShower(
          *dynamic_pointer_cast<JetEnergyLoss>(shared_from_this()));
    } else {
      JSWARN << "No proper external Parton Shower Generator attached ...";
      exit(-1);
    }
    ...
    if (GetPartonShowerGenerator()) {
      ...
      JSDEBUG
          << "--> Use DoShower() provided from PSG reading in full intial "
             "shower to do Parton shower stored in PartonShower Graph class";
    ...
    JSWARN << "NO Initial Hard Parton/or (ISR) shower for Parton shower "
              "received ...";
    exit(-1);
  }
...
void JetEnergyLoss::InitPerEvent() {
  if (GetShowerInitiatingParton() && !useShower) {
    ...
    if (GetPartonShowerGenerator()) {
      ...
      GetPartonShowerGenerator()->DoInitPerEvent(
          *dynamic_pointer_cast<JetEnergyLoss>(shared_from_this()));
    } else {
      JSWARN << "No proper external Parton Shower Generator attached ...";
      exit(-1);
...
void JetEnergyLoss::ExecTime() {
  if (!useShower) {
    ...
    GetPartonShowerGenerator()->DoExecTime(
        *dynamic_pointer_cast<JetEnergyLoss>(shared_from_this()));
  }
}
...
void JetEnergyLoss::FinishPerEvent() {
  if (!useShower) {
    ...
    GetPartonShowerGenerator()->DoFinishPerEvent(
        *dynamic_pointer_cast<JetEnergyLoss>(shared_from_this()));
  }
...
void JetEnergyLoss::DoFinishPerEvent() {
  ...
  // REMARK JP: This is not how a writer/a task should be used, the issue on
  // github
...
void JetEnergyLoss::CalculateTime() {
  // VERBOSE(3) << "Calculate JLoss per time step ... Current (Module) Time = "
...
void JetEnergyLoss::DoExecTime(double currentTime, double deltaT) {
  ...
  //****************************************************************************************************
  // REMARK: Check if this is fully correct, also solve the deltaT issues wrt to
  // module current time ...
  //****************************************************************************************************
  // JP: Check if statement wrt to proper per timestep evolution?
  // Not needed for normal per event exec !? ... Matter checks for time (has to
  // be followed up!!!!)
  ...
  VERBOSESHOWER(7) << "Current time = " << currentTime + deltaT
                   << " with #Input " << pIn.size();
  ...
  for (int i = 0; i < pIn.size(); i++) {
    ...
    // ... !!!!
    SentInPartons(deltaT, currentTime + deltaT, pIn[i].pt(), pInTempModule,
                  pOutTemp);
