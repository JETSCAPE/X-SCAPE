...
JetScapeParticleBase::JetScapeParticleBase(int label, int id, int stat,
  ...
  assert(InternalHelperPythia.particleData.isParticle(id));
  if ((std::abs(pid()) == 1) || (std::abs(pid()) == 2) ||
      (std::abs(pid()) == 3)) {
    set_restmass(0.0);
  } else {
    set_restmass(InternalHelperPythia.particleData.m0(id));
...
JetScapeParticleBase &JetScapeParticleBase::operator=(
    const JetScapeParticleBase &c) {
  fjcore::PseudoJet::operator=(c);
...
ostream &operator<<(ostream &output, JetScapeParticleBase &p) {
  ...
  // output<<p.pt()<<" "<< (fabs (p.rap())>1e-15?p.rap():0)<<" "<< p.phi() <<"
  // "<<p.e()<<" ";
...
Parton::Parton(int label, int id, int stat, const FourVector &p,
  ...
  // cout << "========================== std Ctor called, returning : " << endl
  // << *this << endl;
}
...
Parton::Parton(int label, int id, int stat, double pt, double eta, double phi,
  ...
  // cout << "========================== phieta Ctor called, returning : " <<
  // endl << *this << endl;
}
...
const double Parton::t() {
  ...
  t_parton = e() * e() - px() * px() - py() * py() - pz() * pz() -
             restmass() * restmass();
  // if (t_parton < 0.0) {
  //   JSWARN << " Virtuality is negative, MATTER cannot handle these particles
  //   " << " t = " << t_parton; JSWARN << " pid = "<< pid() << " E = " << e()
  //   << " px = " << px() << " py = " << py() << " pz = " << pz() ;
  // }
  return (t_parton);
  // return (t_) ;
...
bool Hadron::has_no_position() {
  return (x_in_.t() < 1e-6) && (x_in_.x() < 1e-6) && (x_in_.y() < 1e-6) &&
         (x_in_.z() < 1e-6);
}
...
bool Hadron::has_valid_momentum() const {
  ...
  if ((E2minusp2 < rounding_error) && pid() != 22) {
    JSWARN << "Invalid momentum for hadron with label " << plabel() << ", id "
           << pid() << ": E^2 - p^2 = " << E2minusp2;
    return false;
  }
  return true;
}
...
Qvector::Qvector(double pt_min, double pt_max, int npt, double y_min,
                 double y_max, int ny, int norder, int pid, int rapidity_type)
    : pt_min_(pt_min),
      pt_max_(pt_max),
      npt_(npt),
      y_min_(y_min),
      ...
      norder_(norder),
      pid_(pid),
      rapidity_type_(rapidity_type) {
  dpt_ = (pt_max - pt_min) / npt;
  dy_ = (y_max - y_min) / ny;
  hist_.resize(npt, std::vector<std::vector<double>>(
                        ny, std::vector<double>(ncols_, 0.0)));
  gridpT_.reserve(npt);
  gridy_.reserve(ny);
  total_num_ = 0;
...
void Qvector::fill(double pt_in, double y_in, int col_in, double val) {
  ...
  if (idx >= 0 && idx < npt_ && idy >= 0 && idy < ny_ && col_in >= 0 &&
      col_in < ncols_) {
...
void Qvector::set_header(std::string a) { header_ = a; }
