...
int JetScapeTaskSupport::RegisterTask() {
  VERBOSE(1) << "JetScapeTaskSupport::RegisterTask called, answering "
             << CurrentTaskNumber;
  CurrentTaskNumber++;
...
void JetScapeTaskSupport::ReadSeedFromXML() {
  ...
  tinyxml2::XMLElement *RandomXmlDescription =
      JetScapeXML::Instance()->GetElement({"Random"});
  ...
  if (random_seed_ == 0) {
    ...
    JSINFO << "JetScapeTaskSupport found seed 0, using one engine for all and "
              "reseeding to "
           << random_seed_;
    one_generator_per_task_ = false;
  } else {
    JSINFO << "JetScapeTaskSupport found seed " << random_seed_
...
shared_ptr<std::mt19937> JetScapeTaskSupport::GetMt19937Generator(int TaskId) {
  if (!initialized_) {
    JSWARN << "Trying to use JetScapeTaskSupport::GetMt19937Generator before "
              "initialization";
