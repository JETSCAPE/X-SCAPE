...
public:
  ...
  */
  ...
  double GetTotalMomentumFractionPositive(size_t hs){ return TotalMomentumFractionPositive[hs];}
  double GetTotalMomentumFractionNegative(size_t hs){ return TotalMomentumFractionNegative[hs];}
  void SetTotalMomentumFractionPositive(double mom, size_t hs){ TotalMomentumFractionPositive[hs] = mom;}
  void SetTotalMomentumFractionNegative(double mom, size_t hs){ TotalMomentumFractionNegative[hs] = mom;}
  double GetTotalMomentumPositive(size_t hs){ return TotalMomentumPositive[hs];}
  double GetTotalMomentumNegative(size_t hs){ return TotalMomentumNegative[hs];}
  void SetTotalMomentumPositive(double mom, size_t hs){ TotalMomentumPositive[hs] = mom;}
  void SetTotalMomentumNegative(double mom, size_t hs){ TotalMomentumNegative[hs] = mom;}
  void ResizeTotalMomentumVectors(size_t nshowers){
    TotalMomentumPositive.resize(nshowers, 0.0);
    TotalMomentumNegative.resize(nshowers, 0.0);
    TotalMomentumFractionPositive.resize(nshowers, 0.0);
    TotalMomentumFractionNegative.resize(nshowers, 0.0);
  }
  size_t GetNumberOfScatterings(){ return TotalMomentumPositive.size();}
  std::string printer;

  std::vector<Parton> GetRemnants(int hs) { return Remnants[hs]; }
  void PushRemnants(Parton par, int hs) {
    if (Remnants.size() <= hs) {
      Remnants.resize(hs + 1);
    }
    Remnants[hs].push_back(par);
  }
...
private:
  ...
  std::vector<std::vector<Parton>> Remnants;
    
  const int max_colorPerShower = 1000; 
  int max_color, NISRShower = 0;
  std::vector<double> TotalMomentumFractionPositive, TotalMomentumFractionNegative;
  std::vector<double> TotalMomentumPositive, TotalMomentumNegative;
