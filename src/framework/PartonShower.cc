...
unique_ptr<PartonShower> PartonShower::Clone()  // shared_ptr<PartonShower> pS)
{
  ...
  */
  ...
  for (auto currEdge : ebfslist) {
    ...
    pC->new_parton(oldToNew.find(nS)->second, oldToNew.find(nE)->second,
                   make_shared<Parton>(*GetParton(currEdge)));
  }
...
void PartonShower::InsertParton(edge e, shared_ptr<Vertex> v,
                                shared_ptr<Parton> p) {
  node nS = e.source();
  node nE = e.target();
...
void PartonShower::InsertPartonAfter(edge e, shared_ptr<Vertex> v,
                                     shared_ptr<Parton> p) {
  node nS = e.source();
  node nE = e.target();
...
void PartonShower::GetBfsSortedListOfNodesAndEdges(vector<node> &nl,
                                                   vector<edge> &el) {
  bfs search;
  search.reset();
  search.scan_whole_graph(true);
  ...
  for (Eitt = search.tree_edges_begin(), Eendt = search.tree_edges_end();
       Eitt != Eendt; ++Eitt) {
    edge eS = *Eitt;
    el.push_back(eS);
  }
...
double PartonShower::GetNodeTime(node n) { return GetVertex(n)->x_in().t(); }
...
PartonShower::~PartonShower() {
  ...
  pFinal.clear();
  pAll.clear();
  eFinal.clear();  // pVec.clear();vVec.clear();
}
...
void PartonShower::SaveAsGV(string fName) {
  ...
  for (eIt = edges_begin(), eEnd = edges_end(); eIt != eEnd; ++eIt) {
    ...
    // "<<label<<stream1.str()<<stream.str()<<label2<<endl; else
    gv << to_string(eIt->source().id()) + "->" + to_string(eIt->target().id())
...
void PartonShower::SaveAsGraphML(string fName) {
  ...
  g << "<key id=\"nlabel\" for=\"node\" attr.name=\"label\" "
       "attr.type=\"string\"/>"
    << endl;
  // g<<"<key id=\"nx\" for=\"node\" attr.name=\"x\"
  // attr.type=\"double\"/>"<<endl; g<<"<key id=\"ny\" for=\"node\"
  // attr.name=\"y\" attr.type=\"double\"/>"<<endl; g<<"<key id=\"nz\"
  // for=\"node\" attr.name=\"z\" attr.type=\"double\"/>"<<endl;
  g << "<key id=\"nt\" for=\"node\" attr.name=\"t\" attr.type=\"double\"/>"
    << endl;
  g << "<key id=\"nzg\" for=\"node\" attr.name=\"zg\" attr.type=\"double\"/>"
    << endl;

  // g<<"<key id=\"elabel\" for=\"edge\" attr.name=\"label\"
  // attr.type=\"string\"/>"<<endl; g<<"<key id=\"epl\" for=\"edge\"
  // attr.name=\"plabel\" attr.type=\"int\"/>"<<endl; g<<"<key id=\"epid\"
  // for=\"edge\" attr.name=\"pid\" attr.type=\"int\"/>"<<endl; g<<"<key
  // id=\"estat\" for=\"edge\" attr.name=\"pstat\" attr.type=\"int\"/>"<<endl;
  ...
  for (eIt = edges_begin(), eEnd = edges_end(); eIt != eEnd; ++eIt) {
    ...
    // g<<"<data
    // key=\"elabel\">"<<to_string((pMap[*eIt]->pt()))<<"</data>"<<endl;
