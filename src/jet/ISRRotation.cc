...
ISRRotation::ISRRotation() : TotalMomentumFraction(0.0) {
  SetId("ISRRotation");
  VERBOSE(8);
}

ISRRotation::~ISRRotation() { VERBOSE(8); }

void ISRRotation::InitTask() {
  JSINFO << "Intialize ISRRotation ...";

  P_A = GetXMLElementDouble({"Hard", "PythiaGun", "eCM"}) /
  ...
  GSL_RNG =
      std::shared_ptr<gsl_rng>(gsl_rng_alloc(gsl_rng_default), gsl_rng_free);
}

void ISRRotation::DoEnergyLoss(double deltaT, double time, double Q2,
    ...
    if (true) {
      ...
      if (Out.pstat() < 1000) {
        ...
        SampleAgain:
          ...
          if (pT[0] * pT[0] + pT[1] * pT[1] >=
              LatestInitialParton.z() * LatestInitialParton.z()) {
            if (Num < 1000)
              goto SampleAgain;
            pT[0] = 0.0;
            pT[1] = 0.0;
          ...
          (*File1) << "# Latest Parton momentum " << LatestInitialParton.x()
                   << " " << LatestInitialParton.y() << " "
                   << LatestInitialParton.z() << " " << LatestInitialParton.t()
                   << std::endl;
          DefineRotationMatrix((Out.pz() >= 0 ? 1. : -1.));
          AlreadyGeneratedPTForThisShower = true;
          (*File1) << "Generated new (px, py, new pz) : " << pT[0] << " "
                   << pT[1] << " " << LatestPartonNewPz << std::endl;
        ...
        if (Out.plabel() == LatestPartonLabel_Postive) {
          ...
          VERBOSE(2) << "LatestPartonLabel_Postive used "
                     << LatestPartonLabel_Postive;
        }
        if (Out.plabel() == LatestPartonLabel_Negative) {
          AddRemenant(Out, LatestPartonLabel_Negative);
          VERBOSE(2) << "LatestPartonLabel_Negative used "
                     << LatestPartonLabel_Negative;
        }
      ...
      if (lab < 0 && lab > -NPartonPerShower) {
        ...
        // Chooses only the inital MPI partons from Pythia who undergo the
        // scattering
        if (Out.pz() >= 0) {
          double OnShellEnergy =
              sqrt(Out.px() * Out.px() + Out.py() * Out.py() +
                   Out.pz() * Out.pz() + Out.restmass() * Out.restmass());
          ini->CollisionPositiveRotatedMomentum[(-Out.plabel() - 1) / 2] =
              FourVector(Out.px(), Out.py(), Out.pz(), OnShellEnergy);
        }
        ...
        if (Out.pz() < 0) {
          ...
          double OnShellEnergy =
              sqrt(Out.px() * Out.px() + Out.py() * Out.py() +
                   Out.pz() * Out.pz() + Out.restmass() * Out.restmass());
          ini->CollisionNegativeRotatedMomentum[(-Out.plabel() - 1) / 2] =
              FourVector(Out.px(), Out.py(), Out.pz(), OnShellEnergy);
        }
      }

      if ((Out.plabel() == Current_Label || std::abs(Out.pid()) == cid ||
           std::abs(Out.pid()) == bid) &&
          Out.pstat() < 0) {
        VERBOSE(0) << MAGENTA << " iMATTER Pushing particlelabel "
                   << Out.plabel() << " status " << Out.pstat() << " pid "
                   << Out.pid() << " e " << Out.e() << " px " << Out.px()
                   << " py " << Out.py() << " pz " << Out.pz()
      ...
      if (pIn[in].pstat() == 1000) {
        ...
        if (CollisionPositive1.x() == 0 && CollisionNegative1.x() == 0 &&
            CollisionPositive1.y() == 0 && CollisionNegative1.y() == 0) {
          // Don't perform any boost
          return;
        }
        //  else if (CollisionPositive1.x() == CollisionPositive.x() && ) { //
        //  if only positive side doesn't need rotation
        //   CollisionPositive1 = ini->CollisionPositiveMomentum[Index];
        // } else if (CollisionNegative1.t() == 0) { // if only negative side
        ...
        double vx =
            (CollisionPositive.x() + CollisionNegative.x());  // EnergySum;
        double vy =
            (CollisionPositive.y() + CollisionNegative.y());  // EnergySum;
        double vz =
            (CollisionPositive.z() + CollisionNegative.z());  // EnergySum;

        // double EnergySum1 = CollisionPositive1.t() + CollisionNegative1.t();
        double vx1 =
            -(CollisionPositive1.x() + CollisionNegative1.x());  // EnergySum1;
        double vy1 =
            -(CollisionPositive1.y() + CollisionNegative1.y());  // EnergySum1;
        double vz1 =
            -(CollisionPositive1.z() + CollisionNegative1.z());  // EnergySum1;
        ...
        if (!(std::abs(vx - vx1) < 1e-10 && std::abs(vy - vy1) < 1e-10 &&
          ...
          double NewE =
              std::sqrt(NewPx * NewPx + NewPy * NewPy + NewPz * NewPz);
...
void ISRRotation::ResetShower() { AlreadyGeneratedPTForThisShower = false; }
...
double ISRRotation::DisP(double pxGeV, double pyGeV) {
  ...
  // return 2. * std::exp(-2. * s0 * s0 * (px * px + py * py)) * (2. +
  // std::cos(px * (x0 - x1) + py * (xy0 - xy1))
  //           + std::cos(px * (x0 - x2) + py * (xy0 - xy2)) + std::cos(px * (x0
  //           - x3) + py * (xy0 - xy3))
  //           + std::cos(px * (x1 - x2) + py * (xy1 - xy2)) + std::cos(px * (x1
  //           - x3) + py * (xy1 - xy3))
  //           + std::cos(px * (x2 - x3) + py * (xy2 - xy3)));
  double Out = 2.0;
  for (int i = 0; i < NumHotspots - 1; i++) {
    for (int j = i + 1; j < NumHotspots; j++) {
      Out += std::cos(px * (Hotspots[3 * i] - Hotspots[3 * j]) +
                      py * (Hotspots[3 * i + 1] - Hotspots[3 * j + 1]));
    }
  }
...
void ISRRotation::DefineRotationMatrix(double Dir) {
  ...
  if (Dir >= 0) {
    ...
    pz = Signb * std::sqrt(LatestInitialParton_Positive.z() *
                               LatestInitialParton_Positive.z() -
                           px * px - py * py);
  } else {
    px = LatestInitialParton_Negative.x() + pT[0];
    py = LatestInitialParton_Negative.y() + pT[1];
    pz = Signb * std::sqrt(LatestInitialParton_Negative.z() *
                               LatestInitialParton_Negative.z() -
                           px * px - py * py);
  }
...
void ISRRotation::SetLatestInitialParton(double px, double py, double pz,
                                         double E, int Label) {
  if (pz >= 0) {
