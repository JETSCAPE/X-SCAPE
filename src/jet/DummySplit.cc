...
DummySplit::~DummySplit() { VERBOSE(8); }
...
void DummySplit::DoEnergyLoss(double deltaT, double time, double Q2,
  ...
  if (rNum < 0.05 && std::abs(time) < 3) {
    ...
    // double
    // m=sqrt(p.e()*z*z*p.e()-p.pt()*z*p.pt()*z*cosh(p.eta())*cosh(p.eta()));
    ...
    Parton pNew(pp.plabel(), pp.pid(), pp.pstat(), p.pt(), p.eta(), p.phi(),
                p.e(), x);
    Parton pNew2(pp.plabel(), pp.pid(), pp.pstat(), ve.pt(), ve.eta(), ve.phi(),
                 ve.e(), x);
