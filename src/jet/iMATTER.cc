...
const Parton initial(0, 21, 0, 0.0, 0.0, 0.0, 0.0,
                     y);  // a gluon with no momentum at the origin.
...
void iMATTER::InitTask() {
  ...
  RealP_A = GetXMLElementDouble({"Hard", "PythiaGun", "eCM"}) /
            2.0;  /// Assuming symmetric system

  P_A = RealP_A;

  P_B =
      P_A;  /// assuming symmetric system, rewrite for non-symmetric collision.

  if (!P_A) {
    JSWARN << "Initial nucleon energy not found by iMATTER, assuming P_A = "
              "2510 GeV";
    P_A = 2510;
  ...
  pdfpath << getenv("PYTHIA8DIR")
          << "/share/Pythia8/pdfdata";  // usually PYTHIA8DATA leads to xmldoc
                                        // but need pdfdata
  // boost::filesystem::exists();
  if (getenv("PYTHIA8DIR") == NULL) {
    throw std::runtime_error(
        "$PYTHIA8DIR environment variable not set, please make sure the "
        "variable points to the pythia8 installation folder found using "
        "(export PYTHIA8DIR=`pythia8-config --prefix`).");
  }
  ...
  if (!boost::filesystem::exists(pdfpath.str().c_str())) {
    throw std::runtime_error(
        "iMatter cannot find the pdfdata folder of pythia. Tried looking in " +
        pdfpath.str());
  }
  // pdf = new Pythia8::LHAGrid1( 2212, "20", pdfpath.str().c_str(), &info); ///
  // Assuming its a proton
  pdf = new Pythia8::LHAGrid1(
      2212, "20", pdfpath.str(),
      nullptr);  /// Assuming its a proton (since Pythia 8.310)
...
void iMATTER::DoEnergyLoss(double deltaT, double time, double Q2,
  ...
  for (int in = 0; in < pIn.size();
       in++)  /// we continue with the loop charade, even though the framework
              /// is just giving us one parton
  {
    if (std::abs(time - GetMaxT()) <= 1e-10) {
      if (std::abs(pIn[in].pid()) >= cid && std::abs(pIn[in].pid()) != 21 &&
          pIn[in].plabel() < 0) {  //  if it's a parton not treated by iMatter
        if (pIn[in].pz() >= 0) {
          double OnShellEnergy =
              sqrt(pIn[in].px() * pIn[in].px() + pIn[in].py() * pIn[in].py() +
                   pIn[in].pz() * pIn[in].pz() +
                   pIn[in].restmass() * pIn[in].restmass());
          ini->CollisionPositiveMomentum[(-pIn[in].plabel() - 1) / 2] =
              FourVector(pIn[in].px(), pIn[in].py(), pIn[in].pz(),
                         OnShellEnergy);
        }
        ...
        if (pIn[in].pz() < 0) {
          double OnShellEnergy =
              sqrt(pIn[in].px() * pIn[in].px() + pIn[in].py() * pIn[in].py() +
                   pIn[in].pz() * pIn[in].pz() +
                   pIn[in].restmass() * pIn[in].restmass());
          ini->CollisionNegativeMomentum[(-pIn[in].plabel() - 1) / 2] =
              FourVector(pIn[in].px(), pIn[in].py(), pIn[in].pz(),
                         OnShellEnergy);
        }

        FinalRotation->SetLatestInitialParton(
            pIn[in].px(), pIn[in].py(), pIn[in].pz(), std::abs(pIn[in].pz()),
            pIn[in].plabel());
      }
    ...
    if (std::abs(pIn[in].pid()) >= cid &&
        std::abs(pIn[in].pid()) != 21) {  // neglect photons and heavy quarks
      // throw std::runtime_error(" Only light flavors allowed when using
      // iMATTER for now");
    ...
    // Setting the Local assignments, position, momentum, velocity and velocity
    // Mod
    double px = pIn[in].px();
    double py = pIn[in].py();
    double pz = pIn[in].pz();
    ...
    // JSINFO << BOLDYELLOW << " velocityMod = " << velocityMod << " vx = " <<
    // velocity[1] << " vy = " << velocity[2] << " vz = " << velocity[3];
    velocity[0] = velocityMod;

    // JSINFO << BOLDYELLOW << " end location x = " << x_end << " y = " << y_end
    // << " z = " << z_end << " t = " << t_end ;
    ...
    if (!ini) {
      ...
      density_projectile =
          ini->Get_projectile_nucleon_density_lab(time, x, y, z);
    }

    // DEBUG:
    //  std::cout<< " at time " << time << " x = " << x << " y= " << y << " and
    //  z = " << z << " target-density   = " << density_target << endl ;
    //  std::cout<< " at time " << time << " x = " << x << " y= " << y << " and
    //  z = " << z << " projectile-density   = " << density_projectile << endl ;
    ...
    // JSINFO << BOLDYELLOW << " formation time " << pIn[in].form_time() << "
    // end time = " <<  t_end ;
    Current = pIn[in];

    if (pIn[in].pstat() ==
        -1000)  // parton stub from pythia, needs to be reset.
    {
      ...
      if (pIn[in].pz() >= 0.0) {
        ...
        TotalMomentumFraction =
            Hard->GetTotalMomentumFractionPositive() - MomentumFractionCurrent;
      } else {
        TotalMomentum = Hard->GetTotalMomentumNegative() - Current.e();
        TotalMomentumFraction =
            Hard->GetTotalMomentumFractionNegative() - MomentumFractionCurrent;
      }
      MomentumFractionCurrent =
          MomentumFractionCurrent / (1.0 - TotalMomentumFraction);
      ...
      pIn[in].set_stat(-900);  // status for an unstable initial state parton
                               // moving backward in time.

      VERBOSE(1) << MAGENTA << " pSTAT = -1000 leads to new virt = " << t1
                 << " and New formation time = " << pIn[in].form_time();
      VERBOSE(1) << MAGENTA << " TotalMomentumFraction "
                 << TotalMomentumFraction << " MomentumFractionCurrent "
                 << MomentumFractionCurrent << " TotalEnergy "
                 << TotalMomentum + Current.e() << " Current en "
                 << Current.e();
      ...
      if (-pIn[in].plabel() >= NPartonPerShower) {
        JSWARN << "NHard partons: " << -pIn[in].plabel()
               << " allowed: " << NPartonPerShower;
      ...
      FinalRotation->SetLatestInitialParton(pIn[in].px(), pIn[in].py(),
                                            pIn[in].pz(), pIn[in].e(),
                                            pIn[in].plabel());
    ...
    // JSINFO << BOLDYELLOW << "virtuality = " << Current.t() << " split time =
    // " << split_time ; JSINFO << BOLDYELLOW << " new px = " << Current.px() <<
    // " py =  " << Current.py() << " pz = " << Current.pz() << " energy = " <<
    // Current.e();

    // This logic should be worked out better for now
    // We have to do the last splitting two time step before the last step
    // Because one time step is needed to rotate the shower and then one more
    // for pT generation
    if (time < split_time && pIn[in].plabel() == Current_Label &&
        t1 < -Q0 * Q0 - error && std::abs(time - GetMaxT()) > 1e-10 &&
        (time + 2 * deltaT > GetMaxT() + rounding_error)) {
      if (abs(pIn[in].px()) >= rounding_error ||
          abs(pIn[in].py()) >= rounding_error) {
        std::cout.precision(15);
        std::cerr.precision(15);
        JSWARN << " Current parton not along the z-axis ";
        JSWARN << " time = " << time << " MaxT = " << GetMaxT();
        std::cout << " deltaT = " << deltaT << " " << (time - deltaT + error)
                  << " MaxT = " << GetMaxT() << std::endl;
      ...
      // Setting the Local assignments, position, momentum, velocity and
      // velocity Mod OnShellEnergy = sqrt( px*px + py*py + pz*pz);
      px = 0.0;
      py = 0.0;
      ...
      if (TotalMomentumFraction >= 1. || TotalMomentum >= P_A) {
        VERBOSE(2) << " Current TotalMomentumFraction = "
                   << TotalMomentumFraction << " is larger than 1";
        VERBOSE(2) << " P_A = " << P_A;
        goto SkipSampling;
      }
    ...
    RedoSampling:
      ...
      // JSINFO << BOLDYELLOW << " z_frac = " << z_frac << " parent t = " << t
      // << " current t1 = " << t1 << " sibling t2 = " << t2 << " resulting
      // l_perp = " << l_perp_x ;

      // We take the proton going along the z-axis
      // The direction is determined from the Direction of the current
      // double SiblingPT2  = -(zb_frac * (( t1 + zb_frac * pT2) / z_frac - t )
      // + t2) / z_frac; double SiblingPT2  = zb_frac / z_frac * t - zb_frac *
      // t1 / (z_frac * z_frac) - t2 / z_frac - zb_frac * zb_frac * pT2 /
      // (z_frac * z_frac);
      double SiblingPT2 = -(zb_frac / z_frac / z_frac) * t1 +
                          (zb_frac / z_frac) * t - t2 / z_frac;
      ...
      if (SiblingPT2 > 0)
        ...
        if (NumbSampling > 100) {
          JSWARN << " SiblingPT2 negative = " << SiblingPT2 << " z = " << z_frac
                 << " pT2 = " << pT2;
          JSINFO << " t = " << t << " t1 = " << t1 << " t2 = " << t2;
          JSINFO << " Current.pid() = " << Current.pid()
                 << " pid_Par = " << pid_Par << " pid_Sib = " << pid_Sib;
          JSINFO << " E = " << Current.e() << " px = " << Current.px()
                 << " py = " << Current.py() << " pz = " << Current.pz();
          JSINFO << " Current.status = " << Current.pstat();
      ...
      velocity_Sibling[0] = std::sqrt(std::pow(velocity_Sibling[1], 2) +
                                      std::pow(velocity_Sibling[2], 2) +
                                      std::pow(velocity_Sibling[3], 2));

      velocity_Parent[1] = ParentPx / ParentEn;
      velocity_Parent[2] = ParentPy / ParentEn;
      velocity_Parent[3] = ParentPz / ParentEn;

      velocity_Parent[0] = std::sqrt(std::pow(velocity_Parent[1], 2) +
                                     std::pow(velocity_Parent[2], 2) +
                                     std::pow(velocity_Parent[3], 2));
      Hard->SetNISRShower(Hard->GetNISRShower() + 1);
      // Hard->max_color += 1;
      MAX_COLOR += 1;
      Parton Sibling(Current_Label + 1, pid_Sib, 0 + Current_Status, p_Sibling,
                     Current_Location);
      Sibling.set_max_color(Hard->GetMax_Color() +
                            Hard->GetNISRShower() *
                                Hard->GetMax_ColorPerShower());
      ...
      Parton Parent(Current_Label, pid_Par, -900 + Current_Status, p_Parent,
                    Current_Location);
      ...
      double NewTotMomFract =
          TotalMomentumFraction + ParentPlus / (M_SQRT2 * (P_A));
      double NewTotMom = TotalMomentum + ParentEn;
      VERBOSE(2) << BOLDYELLOW << "NewTotMomFract " << NewTotMomFract
                 << " NewTotMom " << NewTotMom << " P_A " << P_A;
      if (NewTotMomFract >= 1. || NewTotMom >= P_A) {
        VERBOSE(2) << BOLDYELLOW
                   << "Skipped iMATTER split because no energy is available in "
                      "the proton eCM";
        goto SkipSampling;
      }
      if (Parent.pz() >= 0) {
    ...
    SkipSampling:
      ...
      // ini->OutputHardPartonMomentum(pOut[iout].e(), pOut[iout].px() ,
      // pOut[iout].py() , pOut[iout].pz(), sign );
    }
  }
...
bool iMATTER::RotateShower(Parton &pIn) {
  if ((pIn.pstat() + 900 == Current_Status || pIn.pstat() == Current_Status) &&
      pIn.pstat() != -1000) {
    VERBOSE(1) << "Rotating parton pstat = " << pIn.pstat()
               << " plabel = " << pIn.plabel() << " px = " << pIn.px()
               << " py = " << pIn.py() << " pz = " << pIn.pz();
    ...
    double En =
        std::sqrt(p_Parton.x() * p_Parton.x() + p_Parton.y() * p_Parton.y() +
                  p_Parton.z() * p_Parton.z());
    double velocity[4];
    ...
    if (std::abs(p_Parton.x()) < error && std::abs(p_Parton.y()) < error &&
        pIn.pstat() + 899 == Current_Status) {
      FinalRotation->SetLatestInitialParton(pIn.px(), pIn.py(), pIn.pz(),
                                            std::abs(pIn.pz()), pIn.plabel());
    }
...
double iMATTER::generate_initial_virt(Parton p, FourVector location,
                                      double max_t) {
  // double density_projectile=0.0;
  // double density_target=0.0;
  // auto ini =
  // JetScapeSignalManager::Instance()->GetInitialStatePointer().lock();

  // if (!ini)
  // {
  //     JSINFO << MAGENTA << "No initial state module, setting the starting
  //     location to "
  //             "0. Make sure to add e.g. trento before PythiaGun.";
  // }
  // else
  // {
  //     density_target = ini->Get_target_nucleon_density_lab( location.t(),
  //     location.x(), location.y(), location.z() ); density_projectile =
  //     ini->Get_projectile_nucleon_density_lab( location.t(), location.x(),
  ...
  double t = Q0 * Q0;
  if (max_t > Q0 * Q0)
    t = invert_Backward_sudakov(r, min_t, max_t);

  return (t);
}

double iMATTER::generate_Forward_virt(Parton p, FourVector location,
                                      double max_t) {
  // double density_projectile=0.0;
  // double density_target=0.0;
  // auto ini =
  ...
  double t = Q0 * Q0;
  if (max_t > Q0 * Q0)
    t = invert_Forward_sudakov(r, min_t, max_t);

  return (t);
}

double iMATTER::invert_Forward_sudakov(double value, double min_t,
                                       double max_t) {
  if ((value <= 0) || (value >= 1)) {
    JSINFO << BOLDRED
           << " error in value passed to sudakov inverter  = " << value;
  ...
  if (value <= 1. / denom) {
    // Debug
    VERBOSE(1) << "Inverse_Forward_sudakov (value <= denom) value = " << value
               << " denom = " << denom << " abs_min_t = " << abs_min_t
               << " abs_max_t = " << abs_max_t;
    return (min_t);
  }
  ...
  while ((diff > approx) && (span > error)) {
    if (loop_counter > 1000)
      throw std::runtime_error(
          " stuck in infinite loop for finding virtuality ");
  ...
  // Debug
  // (*File) << "# Inverse_Forward_sudakov diff = "<< diff << " estimate = "<<
  // estimate << " denom = "<< denom << " numer = "<< numer
  //         << " abs_min_t = " << abs_min_t << " abs_max_t = " << abs_max_t <<
  //         " abs_t = " << abs_t << std::endl;

  return (abs_t);  // returning a time like virtuality t
}
double iMATTER::invert_Backward_sudakov(double value, double min_t,
                                        double max_t) {
  if ((value <= 0) || (value >= 1)) {
    JSINFO << BOLDRED
           << " error in value passed to sudakov inverter  = " << value;
  ...
  if (value <= numer) {
    // Debug
    VERBOSE(1) << "Invert_Backward_sudakov (value <= numer) value = " << value
               << " numer = " << numer << " abs_min_t = " << abs_min_t
               << " abs_max_t = " << abs_max_t;
    return (min_t);
  }
  ...
  // Debug
  // (*File) << "# Inverse_Backward_sudakov diff = "<< diff << " estimate = "<<
  // estimate << " denom = "<< denom << " numer = "<< numer
  //         << " abs_min_t = " << abs_min_t << " abs_t = " << abs_t <<
  //         std::endl;
...
double iMATTER::Forward_Sudakov(double t1, double t2) {
  ...
  if (std::abs(Current.pid()) <= sid)  // A light quark
  {
    ...
    // double logP = LogSud_Pgg( z_min, z_max ) + 2. * nf * LogSud_Pgq( z_min,
    // z_max ); sudakov = std::exp(- logt2 * logP);
  ...
  // (*File) << "# Sud = " << sudakov << " logP = " << logP << " t1 = " << t1 <<
  // " t2= " << t2  << " Current.pid()= " << Current.pid()
  //         << " x2 = " << x2 << " LogSud_Pqq(t1, t2) = " << -(0.5 *
  //         alpha_s((t1 + t2)/2.0) / pi ) * LogSud_Pqq(t1, t2) << "
  //         LogSud_Pqg(t1, t2) = " << -(0.5 * alpha_s((t1 + t2)/2.0) / pi ) *
  //         LogSud_Pqg(t1, t2)
  //         << " LogSud_Pgg(t1, t2) = " << -(0.5 * alpha_s((t1 + t2)/2.0) / pi
  //         ) * LogSud_Pgg(t1, t2) << std::endl;
  return (sudakov);
}
...
double iMATTER::invert_zDist(double value,
                             std::function<double(double, double)> Dist,
                             double t, double denom) {
  if ((value <= 0) || (value >= 1)) {
    JSINFO << BOLDRED
           << " error in value passed to z Distribution inverter  = " << value;
  ...
  while ((diff > approx) && (span > error)) {
    if (loop_counter > 1000)
      throw std::runtime_error(" stuck in infinite loop for finding z ");
...
double iMATTER::generate_z(Parton p, FourVector CurrentLocation, double tp) {
  ...
  if (Current.pid() == gid) {
    ...
    if (r <= ratio) {  // g-> gg
      ...
      std::function<double(double, double)> Fct = [this](double z_max,
                                                         double t) {
        return this->zDist_Pgg_int(z_max, t);
      };
    ...
    for (size_t id = 1; id <= 3; id++) {  // q -> qg
      ...
      if (r <= ratio) {
        ...
        std::function<double(double, double)> Fct = [this](double z_max,
                                                           double t) {
          return this->zDist_Pgq_int(z_max, t);
        };
    ...
    if (r <= ratio) {  // q->qg
      ...
      std::function<double(double, double)> Fct = [this](double z_max,
                                                         double t) {
        return this->zDist_Pqq_int(z_max, t);
      };
      ...
      std::function<double(double, double)> Fct = [this](double z_max,
                                                         double t) {
        return this->zDist_Pqg_int(z_max, t);
      };
...
FoundzVal:
  if (std::isnan(zVal) || zVal == 0.0) {
    JSWARN << BOLDRED << "zVal is not a number zVal = " << zVal << " r = " << r
           << " r1 = " << r1;
    JSWARN << BOLDRED
           << "MomentumFractionCurrent = " << MomentumFractionCurrent;
    JSINFO << BOLDRED << "pid; Curent  = " << Current.pid()
           << " Parent = " << pid_Par << " Sibling = " << pid_Sib;
    JSWARN << BOLDRED << "ratio = " << ratio << " t = " << tp;
    JSWARN << BOLDRED << "denomEach[0] = " << denomEach[0]
           << " denomEach[1] = " << denomEach[1]
           << " denomEach[2] = " << denomEach[2]
           << " denomEach[3] = " << denomEach[3]
           << " denomEach[4] = " << denomEach[4]
           << " denomEach[5] = " << denomEach[5]
...
double iMATTER::generate_L(double form_time) {
  ...
  if (form_time < 0)
    form_time = std::abs(form_time);
  // we use a positve formation time to generate a splitting time and then make
  // that negative before return.
  ...
  // the value of x_high is slightly arbitrary, the erf function is more or less
  // zero at this distance. picking 10*form_time will not lead to any different
  // results
...
inline double iMATTER::P_z_qq(double z) { return Cf * (1. + z * z) / (1. - z); }
...
inline double iMATTER::zDist_Pgg(double y, double t) {
  ...
  return Jac * alpha_s(t) / (2.0 * pi) * P_z_gg(z) *
         PDF(gid, MomentumFractionCurrent / z, t) / z;
}
inline double iMATTER::zDist_Pqq(double y, double t) {
  double siny = std::sin(y / 2.0);
  double z = 1.0 - siny * siny;
  double Jac = std::sqrt(z * (1.0 - z));
  return Jac * alpha_s(t) / (2.0 * pi) * P_z_qq(z) *
         PDF(pid_Par, MomentumFractionCurrent / z, t) / z;
}
inline double iMATTER::zDist_Pqg(double y, double t) {
  double siny = std::sin(y / 2.0);
  double z = 1.0 - siny * siny;
  double Jac = std::sqrt(z * (1.0 - z));
  return Jac * alpha_s(t) / (2.0 * pi) * P_z_qg(z) *
         PDF(gid, MomentumFractionCurrent / z, t) / z;
}
inline double iMATTER::zDist_Pgq(double y, double t) {
  ...
  double z = 1.0 - siny * siny;
  double Jac = std::sqrt(z * (1.0 - z));
  return Jac * alpha_s(t) / (2.0 * pi) * P_z_qq(1.0 - z) *
         PDF(pid_Par, MomentumFractionCurrent / z, t) / z;
}
...
double iMATTER::zDist_Pgg_int(double z_max, double t) {
  ...
  double y_min = 2.0 * std::asin(std::sqrt(1.0 - z_max)),
         y_max = 2.0 * std::asin(std::sqrt(1.0 - z_min));

  std::function<double(double, double)> FctIntegrand =
      [this](double y, double t) { return this->zDist_Pgg(y, t); };
  double Sudakov =
      SingleIntegral(FctIntegrand, t, y_min, y_max, Error,
                     1e0);  // The tolerance is taken to be large for now //
  return Sudakov;
}
double iMATTER::zDist_Pqq_int(double z_max, double t) {
  ...
  std::function<double(double, double)> FctIntegrand =
      [this](double y, double t) { return this->zDist_Pqq(y, t); };
  double Sudakov =
      SingleIntegral(FctIntegrand, t, y_min, y_max, Error,
                     1e0);  // The tolerance is taken to be large for now //
  return Sudakov;
...
double iMATTER::zDist_Pgq_int(double z_max, double t) {
  ...
  std::function<double(double, double)> FctIntegrand =
      [this](double y, double t) { return this->zDist_Pgq(y, t); };
  double Sudakov =
      SingleIntegral(FctIntegrand, t, y_min, y_max, Error,
                     1e0);  // The tolerance is taken to be large for now //
  return Sudakov;
...
double iMATTER::zDist_Pqg_int(double z_max, double t) {
  ...
  std::function<double(double, double)> FctIntegrand =
      [this](double y, double t) { return this->zDist_Pqg(y, t); };
  double Sudakov =
      SingleIntegral(FctIntegrand, t, y_min, y_max, Error,
                     1e0);  // The tolerance is taken to be large for now //
  return Sudakov;
...
inline double iMATTER::P_z_gg_int(double z) {
  return 2.0 * Ca *
         (z * (2.0 - 0.5 * z + z * z / 3.0) + std::log((1.0 - z) / z));
}
...
inline double iMATTER::LogSud_Pgg(double t_min, double t_max) {
  ...
  // return Ca * (8.0 * std::log(t_max / t_min) + ((t_max - t_min) / 3.0) *
  // (-23.0 - 4.0 * t_min / t_max + 2.0 * t_min2 / t_max2 + 12.0 *
  // std::log(t_max / t_min - 1.0)));
  return Ca *
         (67. / 9. - 2. / 3. * pi * pi + (R1 * R1) * (-4. / 9. * R1 + 1.0) -
          8 * R1 + log1 * (11. / 3. + 2.0 * log1) + 4. * gsl_sf_dilog(R1));
}
inline double iMATTER::LogSud_Pqq(double t_min, double t_max) {
  ...
  double R1 = t_min / t_max;
  double log1 = std::log(R1);
  // return Cf * (0.5 * (t_max - t_min) *(-7.0 + 4.0 * std::log(t_max / t_min
  // - 1.0)) + 3.0 * t_min * std::log(t_max/t_min));
  return Cf * (3.0 - pi * pi / 3.0 - 3.0 * R1 + 1.5 * log1 + log1 * log1 +
               2.0 * gsl_sf_dilog(R1));
}
inline double iMATTER::LogSud_Pqg(double t_min, double t_max) {
  ...
  double R1 = t_min / t_max;
  double log1 = std::log(R1);
  // return (t_max + t_min ) / 3.0 - t_min2 * (1.0 / t_max - t_min / t_max2) -
  // t_min * std::log(t_max / t_min);
...
void iMATTER::printout_current() {
  JSINFO << BOLDYELLOW << " testing Current pid = " << Current.pid()
         << " stat = " << Current.pstat() << " label = " << Current.plabel();

  JSINFO << BOLDYELLOW << " Current x = " << Current.x_in().x()
         << " y = " << Current.x_in().y() << " z = " << Current.x_in().z()
         << " t = " << Current.time();

  JSINFO << BOLDYELLOW << " Current px = " << Current.px()
         << " py = " << Current.py() << " pz = " << Current.pz()
         << " E = " << Current.e();

  JSINFO << BOLDYELLOW << " color = " << Current.color()
         << " acolor = " << Current.anti_color() << " virt = " << Current.t();

  JSINFO << BOLDYELLOW << " jet vx = " << Current.jet_v().x()
         << " vy = " << Current.jet_v().y() << " vz = " << Current.jet_v().z()
         << " v = " << Current.jet_v().t();

  JSINFO << BOLDYELLOW << " mean form time = " << Current.mean_form_time()
         << " form time = " << Current.form_time();
...
void iMATTER::SetupIntegration() {
  ...
  for (int i = 0; i < NLegendre / 2; i++) {
    GaussLegendrePoints[NLegendre / 2 - 1 - i] =
        -GaussLegendrePoints[NLegendre / 2 + 1 + i];
    GaussLegendreWeights[NLegendre / 2 - 1 - i] =
        GaussLegendreWeights[NLegendre / 2 + 1 + i];
  }
  ...
  for (int i = 0; i < Nquadrature / 2; i++) {
    StieltjesPoints[Nquadrature / 2 - 1 - i] =
        -StieltjesPoints[Nquadrature / 2 + 1 + i];
    StieltjesWeights[Nquadrature / 2 - 1 - i] =
        StieltjesWeights[Nquadrature / 2 + 1 + i];
  }
  ...
  for (size_t i = 0; i < Nquadrature; i++) {
    for (size_t j = 0; j < Nquadrature; j++) {
      StieltjesDoubleWeights[j + i * Nquadrature] =
          StieltjesWeights[i] * StieltjesWeights[j];
    }
  }

  for (size_t i = 0; i < NLegendre; i++) {
    for (size_t j = 0; j < NLegendre; j++) {
      GaussLegendreDoubleWeights[j + i * NLegendre] =
          GaussLegendreWeights[i] * GaussLegendreWeights[j];
    }
  }
...
double iMATTER::DoubleIntegral(std::function<double(double, double)> &Integrand,
                               double a, double b, double a1, double b1,
                               double &Error, double epsabs) {
  // Integrate a 2d std::function \int_{a}^{b} dx \int_{a1}^{b1}dy f(x,y)  //
  // The cubature is done using NLegendre points for Gauss-Legendre and
  // (2*NLegendre + 1) points for Gauss-Stieltjes // and the error is computed
  ...
  for (size_t i = 1; i < Nquadrature; i += 2) {
    ...
    for (size_t j = 1; j < Nquadrature; j += 2) {
      ...
      gauss_result +=
          GaussLegendreDoubleWeights[(j / 2) + NLegendre * (i / 2)] * fct;
    }
  ...
  if (std::abs(kronrod_result) > 1e-15)
    Error /= std::abs(kronrod_result);

  if (Error > epsabs)
    throw std::runtime_error(
        "Estimate of the cubature error " + std::to_string(Error) +
        " is larger than the target " + std::to_string(epsabs) +
        "\n Try larger target or using more Gauss points");
  return kronrod_result;
}

double iMATTER::SingleIntegral(std::function<double(double, double)> &Integrand,
                               double t, double a, double b, double &Error,
                               double epsabs) {
  ...
  if (Error > epsabs)
    throw std::runtime_error(
        "Estimate of the quadrature error " + std::to_string(Error) +
        " is larger than the target " + std::to_string(epsabs) +
        "\n Try larger target or using more Gauss points");
  return kronrod_result;
}
...
double iMATTER::PDF(int pid, double z, double t) {
  if (pdf->insideBounds(z, t) == 0)
    throw std::runtime_error(" pdf out of bound z = " + std::to_string(z) +
                             " t= " + std::to_string(t) + "\n");
...
void iMATTER::VERBOSE_OUTPUT(std::string st, Parton &pIn) {
  VERBOSE(2) << st << " pid = " << pIn.pid() << " pstat = " << pIn.pstat()
             << " plabel = " << pIn.plabel() << " t = " << pIn.t()
             << " e = " << pIn.e() << " px = " << pIn.px()
             << " py = " << pIn.py() << " pz = " << pIn.pz()
             << " col = " << pIn.color() << " acol = " << pIn.anti_color()
