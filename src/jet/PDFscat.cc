...
void PDFScat::initialize_samplers(double nu, double Q2){
    ...
    outfile.open("pdf_samplers_info.txt", std::ios_base::app); //
        ...
        for (int pi = 0; pi < 9; pi++){
            ...
            if (pi == 0){
                ...
                if (pid_sampled == 21){
                    ...
                    temp_rate = Integrator_LQ(local_energy, pi, pid_sampled, Q2, nu);
                    temp_qhat = Integrator_LQ_QHat(local_energy, pi, pid_sampled, Q2, nu);
                    outfile<<"pi "<<pi<<" energy "<<local_energy<<" rate "<<temp_rate<<" qhat "<<temp_qhat<<std::endl;
                    TF1 *f1 = new TF1("myfunc", functionToIntegrate_LQ, 0, 1, 5); //TODO: WHAT ARE THESE ARGS
                    ...
                    double par0[] = {local_energy, (double)pi, (double)pid_sampled, Q2, nu}; //TODO: CHANGE THE PARAMS OBV
...
double PDFScat::Integrator_LQ(double E, int pi, int flv, double Q2, double nu){
    TF1 f1("myfunc", functionToIntegrate_LQ, 0, 1, 5); //args are min, max, #args -- min and max are overwritten anyway
    ...
    double par0[] = {E, (double)pi, double(flv), Q2, nu}; 
...
double PDFScat::Integrator_LQ_QHat(double E, int pi,int flv, double Q2, double nu){
	TF1 f1("myfunc", functionToIntegrate_LQ_QHat,0, 1, 5);
