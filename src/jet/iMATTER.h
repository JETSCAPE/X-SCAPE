...
class iMATTER : public JetEnergyLossModule<iMATTER> {
 public:
  ...
  double invert_zDist(double value, std::function<double(double, double)> Dist,
 ...
 private:
  ...
  const double z_min_factor =
      0.94;  // this limits the parent momentum to be P_A/z_min_factor
  ...
  int pid_Sib, pid_Par, Color_Sib = 0, Color_Par = 0, AntiColor_Sib = 0,
                        AntiColor_Par = 0;
  ...
  double DoubleIntegral(std::function<double(double, double)> &Integrand,
                        double a, double b, double a1, double b1, double &Error,
                        double epsabs);
  double SingleIntegral(std::function<double(double, double)> &Integrand,
