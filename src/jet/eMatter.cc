...
RegisterJetScapeModule<eMatter> eMatter::reg("eMatter");

static Pythia8::Pythia PythiaFunction("IntentionallyEmpty", false);

bool eMatter::flag_init = 0;

double eMatter::RHQ[60][20] = {{0.0}};    //total scattering rate for heavy quark
double eMatter::RHQ11[60][20] = {{0.0}};  //Qq->Qq
double eMatter::RHQ12[60][20] = {{0.0}};  //Qg->Qg
double eMatter::qhatHQ[60][20] = {{0.0}}; //qhat of heavy quark

double eMatter::distFncB[N_T][N_p1][N_e2] = {{{0.0}}};
double eMatter::distFncF[N_T][N_p1][N_e2] = {{{0.0}}};
double eMatter::distMaxB[N_T][N_p1][N_e2] = {{{0.0}}};
double eMatter::distMaxF[N_T][N_p1][N_e2] = {{{0.0}}};
...
void eMatter::InitTask() {
  ...
  double m_qhat = GetXMLElementDouble({"Eloss", "eMatter", "qhat0"});
  SetQhat(m_qhat);
  ...
  matter_on = GetXMLElementInt({"Eloss", "eMatter", "matter_on"});
  in_vac = GetXMLElementInt({"Eloss", "eMatter", "in_vac"});
  recoil_on = GetXMLElementInt({"Eloss", "eMatter", "recoil_on"});
  broadening_on = GetXMLElementInt({"Eloss", "eMatter", "broadening_on"});
  ...
  qhatB = GetXMLElementDouble({"Eloss", "eMatter", "qhatB"});
  qhatC = GetXMLElementDouble({"Eloss", "eMatter", "qhatC"});
  qhatD = GetXMLElementDouble({"Eloss", "eMatter", "qhatD"});
  tStart = GetXMLElementDouble({"Eloss", "tStart"});
  //run_alphas = GetXMLElementInt({"Eloss", "eMatter", "run_alphas"});
  QhatParametrizationType=GetXMLElementInt({"Eloss", "eMatter", "QhatParametrizationType"});
  hydro_Tc = GetXMLElementDouble({"Eloss", "eMatter", "hydro_Tc"});
  brick_length = GetXMLElementDouble({"Eloss", "eMatter", "brick_length"});
  vir_factor = GetXMLElementDouble({"Eloss", "eMatter", "vir_factor"});
  initial_virtuality_pT = GetXMLElementInt({"Eloss", "eMatter", "initial_virtuality_pT"});
  ...
  JSINFO << MAGENTA << "eMATTER input parameter";
...
void eMatter::WriteTask(weak_ptr<JetScapeWriter> w) {
  VERBOSE(8);
  auto f = w.lock();
  if (!f)
    return;
...
void eMatter::Dump_pIn_info(int i, vector<Parton> &pIn) {
  JSWARN << "i=" << i << " eMATTER -- status: " << pIn[i].pstat()
...
void eMatter::DoEnergyLoss(double deltaT, double time, double Q2,
  ...
  for (int i = 0; i < pIn.size(); i++) {
    ...
    JSDEBUG << MAGENTA << "eMATTER -- status: " << pIn[i].pstat()
        ...
        0.0) /// A parton without a virtuality or formation time, must set...
    {
      ...
      if ((pIn[i].t() < 0.0) &&
          ((pIn[i].form_time() < -0.1 - rounding_error) ||
           (pIn[i].form_time() > -0.1 + rounding_error))) {
        JSWARN << " parton with a negative virtuality was sent to eMATTER and "
      ...
      for (int j=0; j<4; j++) { xLeft[j] = xStart[j]; }
      // cout << "STARTING LEFT " << xLeft[0] << " " << xLeft[1] << " " << xLeft[2] << " " << xLeft[3] << endl;
      xRight[0] = xLeft[0]+1.;
      xRight[1] = xLeft[1]+initVx;
      xRight[2] = xLeft[2]+initVy;
      xRight[3] = xLeft[3]+initVz;
      // cout << "VELOCITY " << initVx << " " << initVy << " " << initVz << endl;
      // cout << "STARTING RIGHT " << xRight[0] << " " << xRight[1] << " " << xRight[2] << " " << xRight[3] << endl;

      // cout << endl;

      //step forward in time by 1 until only xRight is outside
      while (ini->Get_target_nucleon_density_lab(xRight[0], xRight[1], xRight[2], xRight[3]) > MINDENS) {
        xLeft[0]=xRight[0]; xRight[0]+=1.;
        xLeft[1]=xRight[1]; xRight[1]+=initVx;
        xLeft[2]=xRight[2]; xRight[2]+=initVy;
        xLeft[3]=xRight[3]; xRight[3]+=initVz;

        // cout << "\tRIGHT DENS" << ini->Get_target_nucleon_density_lab(xRight[0], xRight[1], xRight[2], xRight[3]) << endl;
        // cout << "\tLEFT " << xLeft[0] << " " << xLeft[1] << " " << xLeft[2] << " " << xLeft[3] << endl;
        // cout << "\tRIGHT " << xRight[0] << " " << xRight[1] << " " << xRight[2] << " " << xRight[3] << endl;
      }
      ...
      // SC: if matter_on = false, set zero virtuality and eMATTER will not do parton shower
      if (matter_on) {
        double oldpz = pIn[i].pz();
        pIn[i].set_t(tQ2); // Also resets momentum!
        // cout << "SET T; PZ WAS " << oldpz << " IS NOW " << pIn[i].pz() << endl;
    ...
    if (in_vac) { // for vaccuum
      ...
      // set the minimum Q0 for eMATTER using Q^2 = qhat * tau  ELSE use the positive value in XML
    ...
    if (pIn[i].t() > Q0 * Q0 + rounding_error ||
          ...
          time) // it is time to split and calculate the effect of scattering
      {
        ...
        cout << "SPLIT at " << time << " starting from " << initR0 << ", split time is " << splitTime << endl;
        cout << "\tPARENT " << pIn[i].pid() << " E " << pIn[i].e() << " MOM " << pIn[i].px() << " " << pIn[i].py() << " " << pIn[i].pz() << endl;
      ...
      if (pIn[i].pid()==21) {
        ...
        qhatLoc = interpolate_pdf_tables(initEner,1,2); //for now just say it's an up quark
      }
      else { qhatLoc = 0; }
	  ...
	  // prob_el=prob_el*ModifiedProbability(QhatParametrizationType, tempLoc, sdLoc, enerLoc, pIn[i].t());
          ...
          if (el_rand > prob_el) { // elastic scattering happens
            ...
            if (std::isnan(pc2[1]) || std::isnan(pc2[2]) ||
              ...
              JSWARN << "recoil in eMATTER instance 1: pc[0]=" << pc2[0]
            ...
            //check if p < E or p > E for remnant? or can E<0 or E>.938?
            if (pc2[0] < 0) { cout << "RECOIL ENERGY NEGATIVE " << pc2[0] << endl; exit(-1); return; }
            if (pc2[0] > 0.938) { cout << "REMNANT ENERGY NEGATIVE " << 0.938-pc2[0] << endl; exit(-1); return; }
            double psq = pc2[1]*pc2[1] + pc2[2]*pc2[2] + pc2[3]*pc2[3];
            double eremn = 0.938-pc2[0];
            // if (eremn > 0.938/2) { cout << "REMNANT HAS MORE THAN HALF ENUC " << eremn << endl; exit(-1); return; }
            // if (eremn*eremn < psq) { cout << "REMNANT ESQ < PSQ " << eremn << " " << psq << endl; exit(-1); return; }
            ...
            if (ZeroOneDistribution(*GetMt19937Generator()) < ((double) targZ)/((double) targA)) { //hit proton
                switch (pid2) {
                  ...
                  case 1: //d
                    remnant_pid = 2203; break; //uu diquark left over
                  case 2: //u
                    remnant_pid = 2103; break; //ud diquark left over
                  default: 
                ...
                switch (pid2) {
                  ...
                  case 1: //d
                    remnant_pid = 2103; break; //ud diquark left over
                  case 2: //u
                    remnant_pid = 1103; break; //dd diquark left over
                  default: 
                    remnant_pid = -1*pid2; break; //need to preserve overall strangeness etc
                }
            }
            ...
            cout << "ELASTIC SCATTERING  AT T=" << el_time << endl;
            cout << "RECOIL " << pid2 << " E " << pc2[0] << " MOM " << pc2[1] << " " << pc2[2] << " " << pc2[3] << endl;
            cout << "REMNANT " << remnant_pid << " E " << remnant.p(0) << " MOM " << remnant.p(1) << " " << remnant.p(2) << " " << remnant.p(3) << endl;
        ...
        cout << "\tSPLIT DAUGHTER A " << pid_a << " E " << newp[0] << " MOM " << newp[1] << " " << newp[2] << " " << newp[3] << endl;
        ...
        if (iSplit != 3) // not a photon
        {
          ...
          cout << "\tSPLIT DAUGHTER B " << pid_b << " E " << newp[0] << " MOM " << newp[1] << " " << newp[2] << " " << newp[3] << endl;
        ...
        } else // is a photon
        {
          ...
          //JSINFO << BOLDYELLOW << " A photon was made in eMATTER with px = " << pOut[iout].px() << " and sent to the framework " ;
        }
...
double eMatter::generate_vac_t(int p_id, double nu, double t0, double t,
                              double loc_a, int is) {
...
double eMatter::generate_vac_t_w_M(int p_id, double M, double nu, double t0,
...
double eMatter::generate_vac_z(int p_id, double t0, double t, double loc_b,
                              double nu, int is) {
...
double eMatter::generate_vac_z_w_M(int p_id, double M, double t0, double t,
...
double eMatter::generate_L(double form_time) {
...
double eMatter::sudakov_Pgg(double g0, double g1, double loc_c, double E) {
  double sud, g;
  int blurb;
...
double eMatter::sud_val_GG(double h0, double h1, double h2, double loc_d,
                          double E1) {
...
double eMatter::sud_z_GG(double cg, double cg1, double loc_e, double l_fac,
                        double E2) {
...
double eMatter::P_z_gg_int(double cg, double cg1, double loc_e, double cg3,
...
double eMatter::sudakov_Pqq(double q0, double q1, double loc_c, double E) {
  double sud, q;
...
double eMatter::sudakov_Pqq_w_M_vac_only(double M, double q0, double q1,
...
double eMatter::sud_val_QQ(double h0, double h1, double h2, double loc_d,
                          double E1) {
  double val, h, intg, hL, hR, diff, intg_L, intg_R, t_form, span;
...
double eMatter::sud_val_QQ_w_M_vac_only(double M, double h0, double h1,
...
double eMatter::sud_z_QQ(double cg, double cg1, double loc_e, double l_fac,
                        double E2) {
...
double eMatter::sud_z_QQ_w_M_vac_only(double M, double cg, double cg1,
...
double eMatter::P_z_qq_int(double cg, double cg1, double loc_e, double cg3,
                          double l_fac, double E2) {
  double t_q1, t_q3, t_q4, t_q6, t_q8, t_q9, t_q12, q_q1, q_q4, q_q6, q_q9,
      q_q11, qL, tau, res;
...
double eMatter::sudakov_Pqp(double g0, double g1, double loc_c, double E) {
  double sud, g;
  int blurb;
...
double eMatter::sud_val_QP(double h0, double h1, double h2, double loc_d,
                          double E1) {
  double val, h, intg, hL, hR, diff, intg_L, intg_R, t_form, span;
  int blurb;
...
double eMatter::P_z_qq_int_w_M_vac_only(double M, double cg, double cg1,
...
double eMatter::sud_z_QP(double cg, double cg1, double loc_e, double l_fac,
                        double E2) {
...
double eMatter::P_z_qp_int(double cg, double cg1, double loc_e, double cg3,
                          double l_fac, double E2) {
...
double eMatter::sudakov_Pqg(double g0, double g1, double loc_c, double E) {
  double sud, g;
  int blurb;
...
double eMatter::sudakov_Pqg_w_M(double M, double g0, double g1, double loc_c,
                               double E) {
  double sud, g;
  int blurb;
...
double eMatter::sud_val_QG(double h0, double h1, double h2, double loc_d,
                          double E1) {
  double val, h, intg, hL, hR, diff, intg_L, intg_R, t_form, span;
  int blurb;
...
double eMatter::sud_val_QG_w_M(double M, double h0, double h1, double h2,
...
double eMatter::sud_z_QG(double cg, double cg1, double loc_e, double l_fac,
                        double E2) {
...
double eMatter::sud_z_QG_w_M(double M, double cg, double cg1, double loc_e,
...
double eMatter::P_z_qg_int(double cg, double cg1, double loc_e, double cg3,
                          double l_fac, double E2) {
...
double eMatter::P_z_qg_int_w_M(double M, double cg, double cg1, double loc_e,
...
double eMatter::alpha_s(double q2) {
...
double eMatter::profile(double zeta) {
  double prof;
...
// double eMatter::fillQhatTab(double y) {
...
// double eMatter::GeneralQhatFunction(int QhatParametrization, double Temperature, double EntropyDensity, double FixAlphas, double Qhat0, double E, double muSquare)
// {
...
// double eMatter::RunningAlphaS(double muSquare)
// {
//   int ActiveFlavor=3;
...
double eMatter::VirtualityQhatFunction(int QhatParametrization,  double enerLoc, double muSquare)
{
...
// double eMatter::ModifiedProbability(int QhatParametrization, double tempLoc, double sdLoc, double enerLoc, double muSquare)
// {
...
//used in case QhatParametrization=7 of VirtualityQhatFunction
double eMatter::IntegralPDF(double xB, double a, double b)
{
...
// double eMatter::fncQhat(double zeta) {
//   if (in_vac)
//     return (0.0);
...
double eMatter::fncQhat(double zeta) {
  ...
  // double avrQhat = qhatTab1D[indexZeta]*VirtualityQhatFunction(QhatParametrizationType, initEner, tscale);
  double avrQhat = interpolate_pdf_tables(initEner, pPid, 2) * VirtualityQhatFunction(QhatParametrizationType, initEner, tscale);
  // cout << "!!!!!!!!!!!!!!! fncQhat " << avrQhat << endl;
  return (avrQhat);
}
...
// double eMatter::fncAvrQhat(double zeta, double tau) {
...
void eMatter::flavor(int &CT, int &KATT0, int &KATT2, int &KATT3,
...
void eMatter::colljet22(int CT, double temp, double qhat0ud, double v0[4],
...
double eMatter::Mqc2qc(double s, double t, double M) {
...
double eMatter::Mgc2gc(double s, double t, double M) {
...
void eMatter::trans(double v[4], double p[4]) {
...
void eMatter::transback(double v[4], double p[4]) {
  double vv = sqrt(v[1] * v[1] + v[2] * v[2] + v[3] * v[3]);
  double ga = 1.0 / sqrt(1.0 - vv * vv);
  double ppar = p[1] * v[1] + p[2] * v[2] + p[3] * v[3];
  double gavv = (-ppar * ga / (1.0 + ga) - p[0]) * ga;
...
void eMatter::rotate(double px, double py, double pz, double pr[4], int icc) {
...
double eMatter::solve_alphas(double var_qhat, double var_ener, double var_temp) {
...
double eMatter::fnc0_alphas(double var_alphas, double var_qhat, double var_ener,
                           double var_temp) {
...
double eMatter::fnc0_derivative_alphas(double var_alphas, double var_qhat,
...
void eMatter::read_pdf_collision_tables() {
  ...
  if (!fqqhat.is_open()) {
    ...
    for (e=TABLEEMIN; e<=TABLEEMAX; e+=TABLEDE) { 
      fqqhat >> tmp_fread;
      pdf_q_qhat.push_back(tmp_fread * pow(0.1973,3));
    }
  }
  fqqhat.close();
  ...
  if (!fgqhat.is_open()) {
    ...
    for (e=TABLEEMIN; e<=TABLEEMAX; e+=TABLEDE) { 
      fgqhat >> tmp_fread;
      pdf_g_qhat.push_back(tmp_fread * pow(0.1973,3));
    }
  }
  fgqhat.close();
}
