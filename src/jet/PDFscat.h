...
class PDFScat {
	public:
		...
		void get_sample(int energy_index, int process_index, double (&V)[5], double EO);
		double get_qhat(int energy_index, int process_index);
		void GetEIndexRange();
		void initialize_samplers();
		double Integrator_LQ(double E, int pi);
		double Integrator_LQ_QHat(double E, int pi);
