...
// Create e+e- -> qqbar processes with Pythia and return the two inital partons
// with set virtualities
...
void epemGun::InitTask() {
  ...
  readString(
      "WeakSingleBoson:ffbar2gmz=on");  // Scattering f fbar → gamma^*/Z^0, with
                                        // full interference between the gamma^*
                                        // and Z^0
  readString("WeakDoubleBoson:all=on");  // Common switch for the group of pair
                                         // production of gamma^*/Z^0 and W^+-
  readString("WeakBosonExchange:all=on");  // Common switch for the group of
                                           // gamma^*/Z^0 or W^+- exchange
...
void epemGun::Exec() {
  ...
  do {
    ...
    for (int parid = 0; parid < event.size(); parid++) {
      ...
      if ((std::abs(particle.id()) > 1100) &&
        ...
        particle.id(-1 * particle.id() /
                    1000);  // Changed from previous two lines.
      }
  ...
  if (p62.size() == 2 && std::abs(p62[0].e() - 0.5 * eCM) < 0.001 &&
      std::abs(p62[1].e() - 0.5 * eCM) < 0.001 && std::abs(p62[0].id()) < 6 &&
      std::abs(p62[1].id()) < 6) {
    ...
    for (int pass = 0; pass < 2; ++pass) {
      ...
      double min_vir = (0.5 * QS * QS) *
                       (1.0 + std::sqrt(1.0 + 4.0 * mass * mass / (QS * QS)));
      ...
      if (max_vir <= QS * QS) {
        ...
        if (max_vir >=
            (QS * QS / 2.) *
                (1.0 + std::sqrt(1.0 + 2.0 * mass * mass / (QS * QS / 2.)))) {
          double g =
              (QS * QS / 2.) *
              (1.0 + std::sqrt(1.0 + 2.0 * mass * mass / (QS * QS / 2.)));
          numer =
              exp(-1.0 * (Cf / 2.0 / pi) *
                  sud_val_QG_w_M(mass, (QS * QS / 2.), g, max_vir, 0.5 * eCM));
        }
        ...
        if (numer > random) {
          ...
          do {
            ...
            if (t_mid <
                (QS * QS / 2.) * (1.0 + std::sqrt(1.0 + 2.0 * mass * mass /
                                                            (QS * QS / 2.)))) {
              denom = 1.0;
            } else {
              double g =
              ...
              denom = exp(
                  -1.0 * (Cf / 2.0 / pi) *
                  sud_val_QG_w_M(mass, (QS * QS / 2.), g, t_mid, 0.5 * eCM));
            }
          ...
          } while ((abs(diff) > s_approx) &&
                   (abs(t_hi - t_low) / t_hi > s_error));
    ...
    if (eCM > sqrt(modm_sq1) + sqrt(modm_sq2)) {
      // Check viability condition; should always be satisfied in the massless
      // case if virfactor < 1
      double pnew = 0.5 *
                    sqrt((eCM * eCM - modm_sq1 - modm_sq2) *
                             (eCM * eCM - modm_sq1 - modm_sq2) -
                         4. * modm_sq1 * modm_sq2) /
                    eCM;
  ...
  for (int np = 0; np < p62.size(); ++np) {
    ...
    if (p62.size() == 2 && std::abs(particle.id()) < 6) {
      double mean_form_time =
          (2. * ptn->e()) /
          (ptn->e() * ptn->e() - ptn->px() * ptn->px() - ptn->py() * ptn->py() -
...
double epemGun::sud_val_QG_w_M(double M, double h0, double h1, double h2,
                               double E1) {
  ...
  if ((diff > approx) || (span > error)) {
    intg = sud_val_QG_w_M(M, h0, h1, h, E1) + sud_val_QG_w_M(M, h0, h, h2, E1);
  }
...
double epemGun::sud_z_QG_w_M(double M, double cg, double cg1, double E2) {
  ...
  /*if(M < 1.){
    ...
    double t17 = -1.0 / t2 * (3.0 * cg1 - 6.0 * cg + 4.0 * t6 * cg1 - 4.0 * t11
  * cg1) / 2.0; if (t17 < 0.0) { cerr << "ERROR: t17 negative in sud_z_QG_w_M =
  " << t17 << endl; throw std::runtime_error("ERROR: medium contribution
  negative in sud_z_QG_w_M");
    }
    return t17;
  }*/
