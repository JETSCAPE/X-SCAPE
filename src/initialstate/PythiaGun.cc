...
void PythiaGun::InitTask() {
  ...
  //min bias flag for AA
  minBias = GetXMLElementInt({"Hard", "PythiaGun", "min_bias"});
...
void PythiaGun::ExecuteTask() {
  ...
  //setting min bias run for getting all hard scatterings in AA collision
  int Ncoll = 1;
  if (ini && minBias) Ncoll = ini->GetNcoll();
  JSINFO <<  "Total Ncoll = " << Ncoll;

  //loop over Ncoll
  for(int iNcoll=0; iNcoll<Ncoll; iNcoll++){
    //JSINFO << "Starting hard process #" << iNcoll;
    bool flag62 = false;
