...
void EPGun::InitTask() {
  ...
  if (photoproduction) {
    ...
    // readString("PDF:pHardSet = LHAPDF6:PDF4LHC21_40"); //for special PDF
    // setting
  }
  ...
  softMomentumCutoff =
      GetXMLElementDouble({"Hard", "EPGun", "softMomentumCutoff"});
  initial_virtuality_pT =
      GetXMLElementInt({"Eloss", "Matter", "initial_virtuality_pT"});
...
void EPGun::ExecuteTask() {
  ...
  do {
    ...
    if (Q2 < Q2min or Q2 > Q2max)
      continue;
    if (W2 < W2min or W2 > W2max)
      continue;

    // JSINFO << "Q2 = " << Q2 << "; W2 = " << W2 << "; x = " << x << "; y = "
    // << y;
  ...
  // JSINFO << "pQuark Initial: " << pQuarkI.px() << " " << pQuarkI.py() << " "
  // << pQuarkI.pz() << " "; JSINFO << "pQuark Final: " << pQuarkF.px() << " "
  // << pQuarkF.py() << " " << pQuarkF.pz() << " "; JSINFO << "pPhoton: " <<
  // pPhoton.px() << " " << pPhoton.py() << " " << pPhoton.pz() << " "; JSINFO
  // << "Breit frame test: " << pBreit2.px() << " " << pBreit2.py() << " " <<
  // pBreit2.pz() << " ";
  const double QS = 0.9;
  ...
  for (int np = 0; np < p62.size(); ++np) {
    ...
    if (breitVir and particle.status() == 62 and !photoproduction) {
      ...
      double min_vir =
          (QS * QS / 2.0) *
          (1.0 + std::sqrt(1.0 + 4.0 * particle.m() * particle.m() / QS / QS));
      double tQ2 = 0.;

      // using z axis for pT since thats the axis the photon quark collision
      // happens on
      ...
      if (max_vir <= QS * QS) {
        ...
        if (abs(particle.id()) == 4 || abs(particle.id()) == 5) {
          if (max_vir > min_vir) {
            tQ2 = matterHelper.generate_vac_t_w_M(particle.id(), particle.m(),
                                                  nu, QS * QS / 2.0, max_vir, 0,
                                                  iSplit);
          } else {
            tQ2 = QS * QS;
          }
        ...
        if (sqrt(tQ2) > particle.pAbs() /*or sqrt(tQ2) > partp.pAbs()*/)
          tQ2 = min_vir;
      }

      // applying virtuality change to the parton
      // JSINFO << BOLDYELLOW << "Particle with ID: " << particle.id();
      // JSINFO << BOLDYELLOW << "initial momentum: " << particle.px() << " " <<
      // particle.py() << " " << particle.pz(); JSINFO << BOLDYELLOW << "breit
      // momentum: " << partp.px() << " " << partp.py() << " " << partp.pz();
      // JSINFO << BOLDYELLOW << "Virtuality: " << sqrt(tQ2) << " ";

      double scale = sqrt(particle.e() * particle.e() - tQ2 - particle.m2()) /
                     particle.pAbs();
