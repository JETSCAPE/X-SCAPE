...
void CascadeTest::InitTask() { JSINFO << "Cascade Test Init() dummy ..."; }

void CascadeTest::CalculateTime() {
  VERBOSE(2) << "CascadeTest::CalculateTime() wait 100ms ... current time = "
...
void CascadeTest::ExecTime() {
  ...
  if (GetMainClock()->GetCurrentTime() < 2) {
    cout << "CascadeTest::ExecTime(): Current Main Clock Time = "
         << GetMainClock()->GetCurrentTime() << endl;
    cout << "CascadeTest::ExecTime(): Print Histories via vector<any> "
            "eLossHistories = "
            "QueryHistory::Instance()->GetHistoryFromModules(\"JetEnergyLoss\")"
         << endl;
...
void CascadeTest::FinishPerEvent() { hList.clear(); }

any CascadeTest::GetHistory() { return any(hList); }
void CascadeTest::GetBulkInfo(Jetscape::real t, Jetscape::real x,
