...
class ThermalPartonSampler {
 public:
   ...
   *
   * Generates partons according to the specified parameters within a
   * brick-shaped hypersurface defined by the length (L) and width (W)
   * parameters. Partons are sampled according to Fermi-Dirac distributions for
   * light (u, d) and strange quarks.
   *
   * @details
  ...
  std::unordered_map<double, std::vector<std::vector<double>>>&
  getCacheCDFLight() {
    return CacheCDFLight;
  }
  std::unordered_map<double, std::vector<std::vector<double>>>&
  getCacheCDFStrange() {
    return CacheCDFStrange;
  }
 ...
 private:
   ...
   *
   * This function calculates the integral of the Fermi-Dirac distribution,
   * which is used to determine the equilibrium number density of fermions at a
   * given temperature (T) and mass (m). The calculation is performed using a
   ...
   * either light or strange quarks. The CDF values are stored in the
   * appropriate table (CDFTabLight or CDFTabStrange).
   *
   * @param T The temperature.
   ...
   * strange) using the cumulative distribution function (CDF) table. The
   * sampling is performed by generating a random probability and finding the
   * corresponding momentum magnitude using binary search. The momentum vector
   * is then generated with a random direction.
   *
   ...
   *
   * @param v A vector containing the components of the velocity (v0, vx, vy,
   * vz).
   ...
   *
   * @param x A 4-element vector representing the original 4-vector to be
   * boosted.
   * @param BoostMatrix A 4x4 matrix representing the Lorentz boost matrix.
   * @return A 4-element vector containing the components of the boosted
   * 4-vector.
   */
   ...
   * determined by the temperature (`T`), quark type (`quark`), and
   * configuration
