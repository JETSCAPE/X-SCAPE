...
RegisterJetScapeModule<HybridHadronization>
  ...
  if (hadronization) {
    ...
    if (RandomXmlDescription) {
      ...
      if (xmle) {
        ...
        JSWARN << "No <seed> element found for <Random> element in xml, "
                  "seeding to 0";
      }
      if (xml_uintin < std::numeric_limits<unsigned int>::max()) {
        rand_seed = xml_uintin;
      }
      xml_intin = std::numeric_limits<unsigned int>::max();
    } else {
    ...
    if (run_successfully) {
      ...
      for (int iHad = 0; iHad < HH_hadrons.num(); ++iHad) {
        ...
        if ((HH_hadrons[iHad].is_final()) && (HH_hadrons[iHad].parh() == -1) &&
          ...
          std::vector<int> py_remn_parents = HH_hadrons[iHad].parents;
...
void HybridHadronization::recomb() {
  ...
  for (int i_pt = 0; i_pt < HH_showerptns.num(); ++i_pt) {
    ...
    //else parton is a gluon, decay into q-qbar and stick those into the collection; set parent id appropriately for both
      ...
      HH_showerptns[i_pt].mass(2. * xmq +
                               (gmax - 2. * xmq) * ran()); // gluon virtuality
    ...
    for (int q2 = 0; q2 < showerquarks.num() + HH_thermal.num(); ++q2) {
      ...
      if ((considering[0].id() * considering[1].id() > 0) &&
        ...
        for (int q3 = q2 + 1; q3 < showerquarks.num() + HH_thermal.num();
          ...
          urho[0] = 0.5 * (pos_rel[0].x() * pos_rel[0].x() / SigRB2 +
                           k_rel[0].x() * k_rel[0].x() * SigRB2 / hbarc2);
          urho[1] = 0.5 * (pos_rel[0].y() * pos_rel[0].y() / SigRB2 +
                           k_rel[0].y() * k_rel[0].y() * SigRB2 / hbarc2);
          urho[2] = 0.5 * (pos_rel[0].z() * pos_rel[0].z() / SigRB2 +
                           k_rel[0].z() * k_rel[0].z() * SigRB2 / hbarc2);
          ulamb[0] = 0.5 * (pos_rel[1].x() * pos_rel[1].x() / SigLB2 +
                            k_rel[1].x() * k_rel[1].x() * SigLB2 / hbarc2);
          ulamb[1] = 0.5 * (pos_rel[1].y() * pos_rel[1].y() / SigLB2 +
                            k_rel[1].y() * k_rel[1].y() * SigLB2 / hbarc2);
          ulamb[2] = 0.5 * (pos_rel[1].z() * pos_rel[1].z() / SigLB2 +
                            k_rel[1].z() * k_rel[1].z() * SigLB2 / hbarc2);
          ...
          if (WigB[1] * recofactor3 >= rndbaryon) {
            ...
            if (numendpoints == 0) { //no parton is an endpoint
              ...
              if (!is_endpoint[0]) {
                ...
                int set_pos, set_endptid;
                if (std::abs(showerquarks[element[1]].pos_str() -
                             showerquarks[element[0]].pos_str()) <=
                    std::abs(showerquarks[element[1]].pos_str() -
                             showerquarks[element[2]].pos_str())) {
                  set_pos = showerquarks[element[0]].pos_str();
                  set_endptid = showerquarks[element[0]].endpt_id();
                } else {
                  set_pos = showerquarks[element[2]].pos_str();
                  set_endptid = showerquarks[element[2]].endpt_id();
                }
                ...
                int set_pos, set_endptid;
                if (std::abs(showerquarks[element[2]].pos_str() -
                             showerquarks[element[0]].pos_str()) <=
                    std::abs(showerquarks[element[2]].pos_str() -
                             showerquarks[element[1]].pos_str())) {
                  set_pos = showerquarks[element[0]].pos_str();
                  set_endptid = showerquarks[element[0]].endpt_id();
                } else {
        ...
        //3D GS Wig. wavefunction
        WigM[0] =
            std::exp(-pos_rel.t() / (2. * SigM2) - k_rel.t() * SigM2 / hbarc2);
        ...
        for (int iME = 0; iME <= maxE_level; ++iME) {
          WigM[1] += std::pow(u[0], iME) * std::exp(-u[0]) /
                     double(std::tgamma(iME + 1));
        } //std::tgamma(iME+1)==factorial(N)
        ...
        if (WigM[1] * recofactor2 >= rndmeson) {
            ...
            if (is_endpoint[0]) {
              ...
              if (perm2[q2] > 0) {
                ...
                showerquarks[showerquarks[element[0]].sibling()].is_strendpt(
                    true);
                showerquarks[showerquarks[element[0]].sibling()].pos_str(
                    0); //HH_thermal[-element[1]].pos_str;
                showerquarks[showerquarks[element[0]].sibling()].endpt_id(
                    1); //HH_thermal[-element[1]].endpt_id;
              }
            }
          }
          ...
          //now setting if baryon is in excited state
          if (WigM[0] * recofactor2 < rndmeson) {
            formedhadron.is_excited(true);
          }
...
void HybridHadronization::set_baryon_id(parton_collection &qrks,
  ...
  if (std::abs(id[0]) < std::abs(id[1])) {
    std::swap(id[0], id[1]);
  }
  if (std::abs(id[1]) < std::abs(id[2])) {
    std::swap(id[1], id[2]);
  }
  if (std::abs(id[0]) < std::abs(id[1])) {
    std::swap(id[0], id[1]);
...
void HybridHadronization::gluon_decay(HHparton &glu, parton_collection &qrks) {
  ...
  if (sum2 > 0.) {
    Betag.Set(Betag.x(), Betag.y(), Betag.z(), 1. / sqrt(sum2));
  }
  ...
  position.Set(glu.x() - Betag.x() * tau, glu.y() - Betag.y() * tau,
               glu.z() - Betag.z() * tau, glu.x_t() + tau);
  q1.pos(position);
  q2.pos(position);
...
void HybridHadronization::stringprep(parton_collection &SP_remnants,
  ...
  std::stable_sort(&SP_remnants[0], (&SP_remnants[SP_remnants.num() - 1]) + 1,
                   [](const HHparton &parton1, const HHparton &parton2) {
                     return (parton1.string_id() < parton2.string_id());
                   });
  //now that the list is sorted based on string id, going to sort the partons in each string based on the position of the partons in the string
  for (int i = 0; i < SP_remnants.num(); ++i) {
    int start, prev_pos, cur_pos, lastfix;
    lastfix = 0;
    if (i == SP_remnants.num() - 1) {
      lastfix = 1;
    }
    cur_pos = SP_remnants[i].string_id();
    if (i == 0) {
      prev_pos = SP_remnants[0].string_id();
      start = 0;
    }
    if (cur_pos != prev_pos || i == SP_remnants.num() - 1) {
      std::stable_sort(&SP_remnants[start], &SP_remnants[i] + lastfix,
                       [](const HHparton &parton1, const HHparton &parton2) {
                         return (parton1.pos_str() < parton2.pos_str());
                       });
      start = i;
      prev_pos = SP_remnants[i].pos_str();
    }
  }
  ...
  for (int par_i = 0; par_i <= SP_remnants.num(); ++par_i) {
    ...
    if (par_i == 0) {
      prev_str = SP_remnants[0].string_id();
      start = 0;
    }
    //updating the string of the most recent parton
    if (par_i < SP_remnants.num()) {
      upd_str = SP_remnants[par_i].string_id();
    } else {
      upd_str = prev_str + 1;
    }
    ...
    if (upd_str == prev_str) {
      current_str.add(SP_remnants[par_i]);
      continue;
    }
    ...
    for (int i = 0; i < current_str.num(); ++i) {
      if (std::abs(current_str[i].id()) <= 6 && (current_str[i].id() != 0)) {
        ++numqrk;
        netqrk += (2 * std::signbit(-current_str[i].id()) - 1);
      } else if (current_str[i].id() == 21) {
        ++numglu;
      } else if ((std::abs(current_str[i].id()) >= 1103) &&
                 (std::abs(current_str[i].id()) <= 5503) &&
                 ((current_str[i].id() / 10) % 10 == 0)) {
        ++numdiqrk;
        netdiqrk += (4 * std::signbit(-current_str[i].id()) - 2);
      }
    }

    //checking/enforcing color neutrality (will attempt to use thermal partons, if any are present, to accomplish this)
    bool fakepartonadded = false;
    HHparton fakeparton;
    if ((netqrk + netdiqrk) % 3 != 0) {
      fakepartonadded = true;
      if (std::abs((netqrk + netdiqrk) % 3) == 1) {
        fakeparton.id(-(2 * std::signbit(-netqrk - netdiqrk) - 1) *
                      (int(1 + 2 * ran())));
      } else { /*std::abs((netqrk+netdiqrk)%3) == 2*/
        fakeparton.id((2 * std::signbit(-netqrk - netdiqrk) - 1) *
                      (int(1 + 2 * ran())));
      }

      int ilast = 0;
      if (std::abs(current_str[current_str.num() - 1].id()) == 21) {
        ilast = current_str.num() - 1;
      } else if (std::abs(current_str[0].id()) == 21) {
        ilast = 0;
      } else {
        ilast = (current_str.num() - 1) / 2;
      }
      ...
      double fake_phi =
          2 * pi *
          ran(); //sqrt(2*0.2*0.2) = sqrt(0.08) = 0.28284271247461900976033774484194
      if (HH_thermal.num() > 0) {

        int qrk_close = -1;
        double dist2min = 999999999999.;
        for (int i = 0; i < HH_thermal.num(); ++i) {
          if ((fakeparton.id() * HH_thermal[i].id() < 0) ||
              HH_thermal[i].is_used()) {
            continue;
          }
          double distnow = current_str[ilast].posDif2(HH_thermal[i]) +
                           (current_str[ilast].x_t() - HH_thermal[i].x_t()) *
                               (current_str[ilast].x_t() - HH_thermal[i].x_t());
          if (distnow < dist2min) {
            qrk_close = i;
            dist2min = distnow;
        ...
        if (qrk_close > -1) {
          HH_thermal[qrk_close].string_id(prev_str);
          HH_thermal[qrk_close].is_remnant(true);
          HH_thermal[qrk_close].is_used(true);
          HH_thermal[qrk_close].used_str(true);
          fakeparton.id(HH_thermal[qrk_close].id());
          fakeparton.orig(1);
          fakeparton.is_remnant(true);
          fakeparton.is_strendpt(true);
          fakeparton.string_id(prev_str);
          fakeparton.pos_str(-1);
          fakeparton.px(HH_thermal[qrk_close].px());
          fakeparton.py(HH_thermal[qrk_close].py());
          fakeparton.pz(HH_thermal[qrk_close].pz());
          fakeparton.e(HH_thermal[qrk_close].e());
          fakeparton.x(HH_thermal[qrk_close].x());
          fakeparton.y(HH_thermal[qrk_close].y());
          fakeparton.z(HH_thermal[qrk_close].z());
          fakeparton.x_t(HH_thermal[qrk_close].x_t());
          //will reposition fake parton within the string in the next repair section...
          ++str_len;
          ++numqrk;
          netqrk += (2 * std::signbit(-fakeparton.id()) - 1);
        } else {
          fakeparton.orig(-1);
          fakeparton.is_remnant(true);
          fakeparton.is_strendpt(true);
          ...
          fakeparton.px(fake_pT * cos(fake_phi));
          fakeparton.py(fake_pT * sin(fake_phi));
          fakeparton.pz(p_fake * dir);
          fakeparton.x(0.);
          fakeparton.y(0.);
          fakeparton.z(0.);
          fakeparton.x_t(0.);
          fakeparton.mass(xmq);
          fakeparton.e(std::sqrt(fakeparton.px() * fakeparton.px() +
                                 fakeparton.py() * fakeparton.py() +
                                 fakeparton.pz() * fakeparton.pz() +
                                 fakeparton.mass() *
                                     fakeparton.mass())); //fakeparton.e(xmq);
          //will reposition fake parton within the string in the next repair section...
          ++str_len;
          ++numqrk;
          netqrk += (2 * std::signbit(-fakeparton.id()) - 1);
    ...
    connections.resize(str_len, std::vector<bool>(str_len, false));
    ...
    if (numqrk + 2 * numdiqrk == 0) { //no quarks, this is a gluon loop
      if (numglu > 1) {               //just chain the gluons into a loop
        ...
        connections[0][str_len - 1] = true;
        connections[str_len - 1][0] = true;
        ...
        qpair[0].par(current_str[0].par());
        qpair[1].par(current_str[0].par());
        qpair[0].is_shower(current_str[0].is_shower());
        qpair[1].is_shower(current_str[0].is_shower());
        qpair[0].is_thermal(current_str[0].is_thermal());
        qpair[1].is_thermal(current_str[0].is_thermal());
        qpair[0].orig(current_str[0].orig());
        qpair[1].orig(current_str[0].orig());
        qpair[0].string_id(current_str[0].string_id());
        qpair[1].string_id(current_str[0].string_id());
        qpair[0].pos_str(0);
        qpair[1].pos_str(0);
        qpair[0].is_remnant(true);
        qpair[1].is_remnant(true);
        if (qpair[0].par() >= 0) {
          HH_shower[qpair[0].par()].is_used(true);
          HH_shower[qpair[0].par()].used_str(true);
          HH_shower[qpair[0].par()].is_decayedglu(true);
        }
        connections.resize(2, std::vector<bool>(2, false));
        connections[0][1] = true;
        connections[1][0] = true;

        //since we work on 'current_str' for color assignments, we need to overwrite it with qpair...
        current_str.clear();
        current_str.add(qpair);
      }
    } else if (
        numqrk + 2 * numdiqrk ==
        2) { //two quarks, since color neutrality is already enforced, numqrk=2 and numdiqrk=0
      if ((std::abs(current_str[0].id()) <= 6) &&
          (std::abs(current_str[current_str.num() - 1].id()) <=
           6)) { //GOOD string!  Don't need to re-sort.
        for (int i = 1; i < str_len - 1; ++i) {
          connections[i][i + 1] = true;
          connections[i][i - 1] = true;
        }
        ...
        connections[str_len - 1][str_len - 2] = true;
          ...
          (std::abs(current_str[0].id()) <= 6) !=
          (std::abs(current_str[current_str.num() - 1].id()) <=
           6)) { //There's a quark on one end of the string, but not the other...
        ...
        if (std::abs(current_str[current_str.num() - 1].id()) <= 6) {
          std::reverse(&current_str[0],
                       (&current_str[current_str.num() - 1]) + 1);
        }
        ...
        if (!fakepartonadded) {
          for (int i = 0; i < current_str.num(); ++i) {
            if (std::abs(current_str[i].id()) <= 6) {
              std::reverse(&current_str[0], &current_str[i + 1]);
              break;
            }
          }
          for (int i = 1; i < current_str.num(); ++i) {
        ...
        numqrk + 2 * numdiqrk ==
        3) { //can get this either by 3quarks (or antiquarks), or by diquark-quark string
      if (numqrk == 3) { //this is a 3 quark junction system
        ...
        bool semi_diqrk = false;
        int firstq_diq = 0;
        for (int i = 0; i < current_str.num() - 1; ++i) {
          if ((std::abs(current_str[i].id()) <= 6) &&
              (std::abs(current_str[i + 1].id()) <= 6)) {
            semi_diqrk = true;
            firstq_diq = i;
            break;
          }
        ...
        if (semi_diqrk) { //we're treating this string configuration as a pseudo-diquark - quark string
          if ((firstq_diq == 0) &&
              (std::abs(current_str[current_str.num() - 1].id()) <=
               6)) { //starts on the (p)diq, ends on the q; good!
            ...
            connections[str_len - 1][str_len - 2] = true;
              ...
              (firstq_diq ==
               current_str.num() -
                   2)) { //starts on the q, ends on the (p)diq; reverse, then good!
            std::reverse(&current_str[0],
                         (&current_str[current_str.num() - 1]) + 1);
            //connecting the two quarks of the p-diq together, and to the next link in the string
            connections[0][1] = true;
            ...
            connections[str_len - 1][str_len - 2] = true;
              ...
              (firstq_diq ==
               current_str.num() -
                   2)) { //(p)diq terminates the string, but the other end isn't properly terminated; fix!
            if (firstq_diq == current_str.num() - 2) {
              std::reverse(&current_str[0],
                           (&current_str[current_str.num() - 1]) + 1);
            }
            ...
            if (!fakepartonadded) {
              for (int i = 2; i < current_str.num(); ++i) {
                if (std::abs(current_str[i].id()) <= 6) {
                  std::reverse(&current_str[i],
                               (&current_str[current_str.num() - 1]) + 1);
                  break;
            ...
            connections[str_len - 1][str_len - 2] = true;
              ...
              (std::abs(current_str[0].id()) <= 6) ||
              (std::abs(current_str[current_str.num() - 1].id()) <=
               6)) { //q terminates the string, but (p)diq doesn't terminate other end; fix!
            ...
            if (!fakepartonadded) {
              ...
              for (int i = k; i < current_str.num(); ++i) {
                if (std::abs(current_str[i].id()) <= 6) {
                  ...
                  std::reverse(&current_str[0], (&current_str[k]) + 1);
                  break;
                }
              }
              for (int i = k + 1; i < current_str.num(); ++i) {
                if (std::abs(current_str[i].id()) <= 6) {
                  std::reverse(&current_str[i],
                               (&current_str[current_str.num() - 1]) + 1);
                  break;
              ...
              if (!diqstartsstr) {
                std::reverse(&current_str[0],
                             (&current_str[current_str.num() - 1]) + 1);
              }      //reversing the string to put the p-diq at the start
            } else { //fake parton has been added, the two actual quarks are must be next to each other
              for (int i = 0; i < current_str.num(); ++i) {
                if (std::abs(current_str[i].id()) <= 6) {
                  if (2 * i <= current_str.num()) {
                    std::reverse(&current_str[0], (&current_str[i + 1]) + 1);
                  } else {
                    std::reverse(&current_str[i],
                                 (&current_str[current_str.num() - 1]) + 1);
                    std::reverse(&current_str[0],
          ...
          if (fakepartonadded) {
            ...
            if ((std::abs(current_str[0].id()) <= 6) &&
              ...
              for (int i = n + 1; i < current_str.num(); ++i) {
                if (std::abs(current_str[i].id()) <= 6) {
                  m = i;
                  break;
                }
              }
              if (n < current_str.num() - m) {
                std::reverse(&current_str[0], (&current_str[n]) + 1);
              } //reverse to 1st quark (from start), else from 2nd quark (to end)
              else {
                std::reverse(&current_str[m],
                             (&current_str[current_str.num() - 1]) + 1);
                std::reverse(&current_str[0],
                             (&current_str[current_str.num() - 1]) + 1);
              }
            ...
            if ((std::abs(current_str[0].id()) <= 6) &&
              ...
              for (int i = current_str.num() - 1; i > 0; --i) {
                if (std::abs(current_str[i].id()) <= 6) {
                  std::reverse(&current_str[i],
                               (&current_str[current_str.num() - 1]) + 1);
                  break;
          ...
          gluchainlen[0] = 0;
          gluchainlen[1] = 0;
          for (int i = pos + 1; i < current_str.num(); ++i) {
            if (current_str[i].id() == 21) {
              ++gluchainlen[0];
            } else {
              pos = i;
              break;
          ...
          if (gluchainlen[0] >= gluchainlen[1]) { //breaking the first chain
            ...
            int k = 0;
            connections[0][1] = true;
            for (int i = k + 1; i < str_len - 1; ++i) {
              connections[i][i + 1] = true;
              connections[i][i - 1] = true;
              if (current_str[i + 1].id() != 21) {
                k = i + 2;
            ...
            if (k < str_len - 1) {
              connections[k][k + 1] = true;
            }
            connections[k][pos] = true;
            connections[k][pos + 1] = true;
            connections[pos][k] = true;
            connections[pos + 1][k] = true;
            for (int i = k + 1; i < str_len - 1; ++i) {
              connections[i][i + 1] = true;
              connections[i][i - 1] = true;
            }
            ...
            connections[k][str_len - 1 - pos] = true;
            connections[k][str_len - 1 - pos - 1] = true;
            connections[str_len - 1 - pos][k] = true;
            connections[str_len - 1 - pos - 1][k] = true;
            if (k > 0) {
              connections[k][k - 1] = true;
            }
            for (int i = k - 1; i > 0; --i) {
        ...
        if ((std::abs(current_str[0].id()) >= 1103) &&
            (std::abs(current_str[0].id()) <= 5503) &&
            ((current_str[0].id() / 10) % 10 == 0) &&
            (std::abs(current_str[current_str.num() - 1].id()) <= 6)) {
          ...
          connections[str_len - 1][str_len - 2] = true;
                   ...
                   (std::abs(current_str[current_str.num() - 1].id()) <=
                    5503) &&
                   ((current_str[current_str.num() - 1].id() / 10) % 10 == 0)) {
        ...
        } else if (((std::abs(current_str[0].id()) >= 1103) &&
                    (std::abs(current_str[0].id()) <= 5503) &&
                    ((current_str[0].id() / 10) % 10 == 0)) ||
                   ((std::abs(current_str[current_str.num() - 1].id()) >=
                     1103) &&
                    (std::abs(current_str[current_str.num() - 1].id()) <=
                     5503) &&
                    ((current_str[current_str.num() - 1].id() / 10) % 10 ==
                     0))) {
          ...
          if (!fakepartonadded) {
            for (int i = 1; i < current_str.num(); ++i) {
              if ((std::abs(current_str[i].id()) >= 1103) &&
                  (std::abs(current_str[i].id()) <= 5503) &&
                  ((current_str[i].id() / 10) % 10 == 0)) {
                std::reverse(&current_str[i],
                             (&current_str[current_str.num() - 1]) + 1);
          ...
          if (!fakepartonadded) {
            for (int i = 0; i < current_str.num(); ++i) {
              if ((std::abs(current_str[i].id()) <= 6) ||
                  ((std::abs(current_str[i].id()) >= 1103) &&
                   (std::abs(current_str[i].id()) <= 5503) &&
                   ((current_str[i].id() / 10) % 10 == 0))) {
                std::reverse(&current_str[0], &current_str[i]);
                break;
              }
            }
            for (int i = 1; i < current_str.num(); ++i) {
            ...
            //now need to ensure that this string starts with the quark
            if ((std::abs(current_str[current_str.num() - 1].id()) >= 1103) &&
                (std::abs(current_str[current_str.num() - 1].id()) <= 5503) &&
                ((current_str[current_str.num() - 1].id() / 10) % 10 == 0)) {
              std::reverse(&current_str[0],
                           (&current_str[current_str.num() - 1]) + 1);
            }
      ...
      if (cutstr) {
        ...
        int new_string_id = 0;
        for (int i = 0; i < SP_remnants.num(); ++i) {
          if (SP_remnants[i].string_id() > new_string_id) {
            new_string_id = SP_remnants[i].string_id();
          }
        }
        for (int i = 0; i < SP_prepremn.num(); ++i) {
          if (SP_prepremn[i].string_id() > new_string_id) {
            new_string_id = SP_prepremn[i].string_id();
          }
        }
        ++new_string_id;
        ...
        if (fakepartonadded) {
          current_str.add(fakeparton);
        }
        std::vector<std::vector<int>> ptn_legs;
        std::vector<bool> valid_leg;
        ...
        for (int i = 0; i < current_str.num(); ++i) {
          if (current_str[i].id() == 21) {
            ...
            tempptn.push_back(i);
            ptn_legs.push_back(tempptn);
            tempptn.clear();
          }
        }
        for (int i = 0; i < ptn_legs.size(); ++i) {
          valid_leg.push_back(true);
        }
        for (int i = 0; i < ptn_legs.size(); ++i) {
          if ((current_str[ptn_legs[i][0]].id() > 0 &&
               current_str[ptn_legs[i][0]].id() <= 6) ||
              (current_str[ptn_legs[i][0]].id() < -6)) {
            qtype_leg.push_back(true);
          } else {
            qtype_leg.push_back(false);
          }
        }
        bool lastleg_gluloop = false;
        while (gluons.size() > 0) {
          double mindist_glu = 999999999999.;
          int glu_min = 0;
          int leg_glu_min = -1;
          double mindist_legs = 999999999999.;
          int leg1_min = -1;
          int leg2_min = -1;
          for (int i = 0; i < valid_leg.size(); ++i) {
            if (valid_leg[i]) {
              leg_glu_min = i;
              break;
            }
          ...
          for (int i = 0; i < ptn_legs.size(); ++i) {
            ...
            for (int j = 0; j < ptn_legs.size(); ++j) {
              ...
              double dist_now =
                  current_str[ptn_legs[i][ptn_legs[i].size() - 1]].pDif2(
                      current_str[ptn_legs[j][ptn_legs[j].size() - 1]]);
              if (dist_now < mindist_legs) {
                mindist_legs = dist_now;
                leg1_min = i;
                leg2_min = j;
              }
            }
            for (int j = 0; j < gluons.size(); ++j) {
              double dist_now =
                  current_str[ptn_legs[i][ptn_legs[i].size() - 1]].pDif2(
                      current_str[gluons[j]]);
              if (dist_now < mindist_glu) {
                mindist_glu = dist_now;
                leg_glu_min = i;
                glu_min = j;
              }
            }
          ...
          if ((num_legs > 0) &&
              ((mindist_glu <= mindist_legs) || (num_legs < 2))) {
            if (leg_glu_min >= 0) {
              ptn_legs[leg_glu_min].push_back(gluons[glu_min]);
              gluons.erase(gluons.begin() + glu_min);
            }
          } else if (num_legs > 0) {
            if ((leg1_min >= 0) && (leg2_min >= 0)) {
              valid_leg[leg1_min] = false;
              valid_leg[leg2_min] = false;
            }
          } else {
            std::vector<int> tempptn;
            tempptn.push_back(gluons[glu_min]);
            ptn_legs.push_back(tempptn);
            tempptn.clear();
            gluons.erase(gluons.begin() + glu_min);
            valid_leg.push_back(true);
            lastleg_gluloop = true;
          }
        }
        ...
        if (lastleg_gluloop) {
          ...
          for (int i = 0; i < ptn_legs[ptn_legs.size() - 1].size(); ++i) {
            if (i % 2 == 0) {
              tempstr.push_back(ptn_legs[ptn_legs.size() - 1][i]);
            } else {
              tempstr.insert(tempstr.begin(), ptn_legs[ptn_legs.size() - 1][i]);
            }
          }
          cutstrings.push_back(tempstr);
          valid_leg[valid_leg.size() - 1] = false;
          --num_legs;
        }
        ...
        while (num_legs > 0) {
          ...
          int leg1_oppq = -1;
          int leg2_oppq = -1;
          double min_dist12_sameq = -1.;
          double min_dist23_sameq = -1.;
          double min_dist13_sameq = -1.;
          double min_dist_oppq = -1.;
          bool checkJ = false;
          bool checkqq = false;
            ...
            for (int j = 0; j < ptn_legs.size(); ++j) {
              ...
              if (qtype_leg[i] == qtype_leg[j]) {
                  ...
                  if (qtype_leg[i] == qtype_leg[k]) {
                    double dist1_now =
                        current_str[ptn_legs[i][ptn_legs[i].size() - 1]].pDif2(
                            current_str[ptn_legs[j][ptn_legs[j].size() - 1]]);
                    double dist2_now =
                        current_str[ptn_legs[j][ptn_legs[j].size() - 1]].pDif2(
                            current_str[ptn_legs[k][ptn_legs[k].size() - 1]]);
                    double dist3_now =
                        current_str[ptn_legs[k][ptn_legs[k].size() - 1]].pDif2(
                            current_str[ptn_legs[i][ptn_legs[i].size() - 1]]);
                    double distsum_now = dist1_now + dist2_now + dist3_now;
                    double distsum_prev =
                        min_dist12_sameq + min_dist23_sameq + min_dist13_sameq;
                    if ((distsum_prev < 0) || (distsum_now < distsum_prev)) {
                      checkJ = true;
                      leg1_sameq = i;
                      leg2_sameq = j;
                      leg3_sameq = k;
                      min_dist12_sameq = dist1_now;
                      min_dist23_sameq = dist2_now;
                      min_dist13_sameq = dist3_now;
                    }
                  }
                }
              } else {
                double dist_now =
                    current_str[ptn_legs[i][ptn_legs[i].size() - 1]].pDif2(
                        current_str[ptn_legs[j][ptn_legs[j].size() - 1]]);
                if ((min_dist_oppq < 0) || (dist_now < min_dist_oppq)) {
                  checkqq = true;
                  leg1_oppq = i;
                  leg2_oppq = j;
                  min_dist_oppq = dist_now;
                }
              }
            }
          }

          if (checkJ && ((min_dist_oppq < 0) ||
                         (min_dist12_sameq + min_dist23_sameq +
                          min_dist13_sameq) < (3. * min_dist_oppq))) {
            std::vector<int> tempstr;
            int sorted_legs[3];
            sorted_legs[0] = leg1_sameq;
            sorted_legs[1] = leg2_sameq;
            sorted_legs[2] = leg3_sameq;
            if (ptn_legs[sorted_legs[1]].size() >
                ptn_legs[sorted_legs[0]].size()) {
              std::swap(sorted_legs[0], sorted_legs[1]);
            }
            if (ptn_legs[sorted_legs[2]].size() >
                ptn_legs[sorted_legs[1]].size()) {
              std::swap(sorted_legs[1], sorted_legs[2]);
            }
            if (ptn_legs[sorted_legs[1]].size() >
                ptn_legs[sorted_legs[0]].size()) {
              std::swap(sorted_legs[0], sorted_legs[1]);
            }
            for (int i = 0; i < ptn_legs[sorted_legs[0]].size(); ++i) {
              tempstr.push_back(ptn_legs[sorted_legs[0]][i]);
            }
            for (int i = ptn_legs[sorted_legs[1]].size() - 1; i >= 0; --i) {
              tempstr.push_back(ptn_legs[sorted_legs[1]][i]);
            }
            for (int i = ptn_legs[sorted_legs[2]].size() - 1; i >= 0; --i) {
              tempstr.push_back(ptn_legs[sorted_legs[2]][i]);
            }
            cutstrings.push_back(tempstr);
            valid_leg[leg1_sameq] = false;
            valid_leg[leg2_sameq] = false;
            valid_leg[leg3_sameq] = false;
            num_legs -= 3;
          } else if (checkqq) {
            std::vector<int> tempstr;
            for (int i = 0; i < ptn_legs[leg1_oppq].size(); ++i) {
              tempstr.push_back(ptn_legs[leg1_oppq][i]);
            }
            for (int i = ptn_legs[leg2_oppq].size() - 1; i >= 0; --i) {
              tempstr.push_back(ptn_legs[leg2_oppq][i]);
            }
            cutstrings.push_back(tempstr);
            valid_leg[leg1_oppq] = false;
            valid_leg[leg2_oppq] = false;
            num_legs -= 2;
          }
        }
        ...
        for (int i = 0; i < cutstrings.size(); ++i) {
          for (int j = 0; j < cutstrings[i].size(); ++j) {
            current_str[cutstrings[i][j]].string_id(i + new_string_id);
            current_str[cutstrings[i][j]].pos_str(j);
          }
        }
        ...
        for (int i = 0; i < str_completed.num(); ++i) {
          if (str_completed[i].PY_par1() >= 0) {
            str_completed[i].PY_par1(str_completed[i].PY_par1() +
                                     SP_prepremn.num());
          }
          if (str_completed[i].PY_par2() >= 0) {
            str_completed[i].PY_par2(str_completed[i].PY_par2() +
                                     SP_prepremn.num());
          }
          if (str_completed[i].PY_dau1() >= 0) {
            str_completed[i].PY_dau1(str_completed[i].PY_dau1() +
                                     SP_prepremn.num());
          }
          if (str_completed[i].PY_dau2() >= 0) {
            str_completed[i].PY_dau2(str_completed[i].PY_dau2() +
                                     SP_prepremn.num());
          }
          if (str_completed[i].col() > 0) {
            str_completed[i].col(str_completed[i].col() + lastused_tag);
          }
          if (str_completed[i].acol() > 0) {
            str_completed[i].acol(str_completed[i].acol() + lastused_tag);
          }

          if (str_completed[i].col() > tagmax) {
            tagmax = str_completed[i].col();
          }
          if (str_completed[i].acol() > tagmax) {
            tagmax = str_completed[i].acol();
          }
        }
      ...
      //**********************************************************************************************************************************************************************************
        ...
        while (gluons.size() > 0) {
          double mindist_glu = 9999999999999.;
          int glu_min = 0;
          int leg_glu_min = -1;
          double mindist_legs = 999999999999.;
          int leg1_min;
          ...
          if ((mindist_glu <= mindist_legs) || (num_legs < 3)) {
            ptn_legs[leg_glu_min].push_back(gluons[glu_min]);
            gluons.erase(gluons.begin() + glu_min);
          } else {
            ...
            for (int j = 0; j < gluons.size(); ++j) {
              double dist_now =
                  current_str[ptn_legs[leg1_min][ptn_legs[leg1_min].size() - 1]]
                      .pDif2(current_str[gluons[j]]);
              if (dist_now < mindist_glu) {
                mindist_glu = dist_now;
                leg_glu_min = leg1_min;
                glu_min = j;
              }
              dist_now =
                  current_str[ptn_legs[leg2_min][ptn_legs[leg2_min].size() - 1]]
                      .pDif2(current_str[gluons[j]]);
              if (dist_now < mindist_glu) {
                mindist_glu = dist_now;
                leg_glu_min = leg2_min;
                glu_min = j;
              }
            }
            std::vector<int> temp;
            temp.push_back(gluons[glu_min]);
            ptn_legs.push_back(temp);
            temp.clear();
            temp.push_back(leg1_min);
            temp.push_back(leg2_min);
            temp.push_back(ptn_legs.size() - 1);
            pseudojuns.push_back(temp);
            temp.clear();
            valid_leg[leg1_min] = false;
            valid_leg[leg2_min] = false;
            valid_leg.push_back(true);
        ...
        while (num_legs >= 3) {
          if (prev_leg < 0) { //this is the first merger
            ...
            for (int i = 0; i < ptn_legs.size(); ++i) {
              for (int j = i + 1; j < ptn_legs.size(); ++j) {
                if (!valid_leg[i]) {
                  continue;
                }
                if (!valid_leg[j]) {
            ...
            fakeglu.id(21);
            fakeglu.orig(-1);
            fakeglu.is_remnant(true);
            fakeglu.string_id(prev_str);
            fakeglu.pos_str(-1);
            fakeglu.px(0.);
            fakeglu.py(0.);
            fakeglu.pz(0.);
            ...
            std::vector<int> temp;
            temp.push_back(current_str.num() - 1);
            ptn_legs.push_back(temp);
            temp.clear();
            temp.push_back(leg1_min);
            temp.push_back(leg2_min);
            ...
            for (int i = 0; i < ptn_legs.size() - num_fakeglu; ++i) {
              ...
              if (qtype_leg[i] != qtype_leg[qtype_leg.size() - 1]) {
                continue;
              }
              double dist_now =
                  current_str[ptn_legs[i][ptn_legs[i].size() - 1]].pDif2(
                      current_str[ptn_legs[leg1_min]
                                          [ptn_legs[leg1_min].size() - 1]]);
              if (dist_now < mindist) {
                prev_leg = leg1_min;
              }
              dist_now = current_str[ptn_legs[i][ptn_legs[i].size() - 1]].pDif2(
                  current_str[ptn_legs[leg2_min]
                                      [ptn_legs[leg2_min].size() - 1]]);
              if (dist_now < mindist) {
                prev_leg = leg2_min;
              }
            }
            ...
            for (int i = 0; i < ptn_legs.size() - num_fakeglu; ++i) {
              ...
              if (qtype_leg[i] == qtype_leg[prev_leg]) {
                continue;
              }
              double dist_now =
                  current_str[ptn_legs[prev_leg][ptn_legs[prev_leg].size() - 1]]
                      .pDif2(current_str[ptn_legs[i][ptn_legs[i].size() - 1]]);
              if (dist_now < mindist) {
                mindist = dist_now;
                leg_min = i;
              }
            ...
            temp.push_back(prev_glu);
            temp.push_back(leg_min);
            temp.push_back(ptn_legs.size() - 1);
            pseudojuns.push_back(temp);
            temp.clear();
            valid_leg[prev_glu] = false;
            valid_leg[leg_min] = false;
            valid_leg.push_back(true);
            ++num_fakeglu;
            --num_legs;
        ...
        connections.clear();
        connections.resize(current_str.num(),
                           std::vector<bool>(current_str.num(), false));
        ...
        for (int i = 0; i < ptn_legs.size(); ++i) {
          if (ptn_legs[i].size() <= 1) {
            continue;
          }
          connections[ptn_legs[i][0]][ptn_legs[i][1]] = true;
          for (int j = 1; j < ptn_legs[i].size() - 1; ++j) {
            connections[ptn_legs[i][j]][ptn_legs[i][j + 1]] = true;
            connections[ptn_legs[i][j]][ptn_legs[i][j - 1]] = true;
          }
          connections[ptn_legs[i][ptn_legs[i].size() - 1]]
                     [ptn_legs[i][ptn_legs[i].size() - 2]] = true;
        }
        ...
        for (int ipJn = 0; ipJn < pseudojuns.size() - 1; ++ipJn) {
          ...
          for (int ipJL = 0; ipJL < pseudojuns[ipJn].size() - 1; ++ipJL) {
            to_connect.push_back(
                ptn_legs[pseudojuns[ipJn][ipJL]]
                        [ptn_legs[pseudojuns[ipJn][ipJL]].size() - 1]);
          }
          to_connect.push_back(
              ptn_legs[pseudojuns[ipJn][pseudojuns[ipJn].size() - 1]][0]);
        ...
        std::vector<int> to_connect;
        for (int ipJL = 0; ipJL < pseudojuns[pseudojuns.size() - 1].size();
             ++ipJL) {
          to_connect.push_back(
              ptn_legs[pseudojuns[pseudojuns.size() - 1][ipJL]]
                      [ptn_legs[pseudojuns[pseudojuns.size() - 1][ipJL]]
                           .size() -
                       1]);
        }
    ...
    if (fakepartonadded && !(numqrk + 2 * numdiqrk >= 4)) {
      current_str.add(fakeparton);
    }
    ...
    std::vector<std::vector<int>> settag;
    for (int i = 0; i < current_str.num(); ++i) {
      settag.push_back({0, 0});
    }
    ...
    int first_ptn = 0;
    if ((numqrk + 2 * numdiqrk > 0) && (current_str[0].id() == 21)) {
      while (current_str[first_ptn].id() == 21) {
        ++first_ptn;
      }
    }
    //setting the flag for the first parton
    if ((current_str[first_ptn].id() > 0 && current_str[first_ptn].id() <= 6) ||
        (current_str[first_ptn].id() < -6)) {
      settag[first_ptn] = {lastused_tag, 0};
    } else if (current_str[first_ptn].id() == 21) {
      settag[first_ptn] = {lastused_tag, ++lastused_tag};
    } else {
      settag[first_ptn] = {0, lastused_tag};
    }
    ...
    while (stack.size() > 0) {
      ...
      if (!labelled[current]) {
        ...
        std::vector<int> connected_all;
        std::vector<int> connected_new;
        int num_pre = 0;
        for (int i = 0; i < connections.size(); ++i) {
          if (connections[current][i]) {
            connected_all.push_back(i);
            if (!labelled[i]) {
              stack.push_back(i);
              connected_new.push_back(i);
            }
            if (i < current) {
              ++num_pre;
            }
        ...
        if ((std::abs(current_str[current].id()) <= 6) ||
            ((std::abs(current_str[current].id()) >= 1103) &&
             (std::abs(current_str[current].id()) <= 5503) &&
             ((current_str[current].id() / 10) % 10 == 0))) {
          //setting tag
          if ((current_str[current].id() > 0 &&
               current_str[current].id() <= 6) ||
              (current_str[current].id() < -6)) {
            if (settag[current][0] > 0) {
              current_str[current].col(settag[current][0]);
            } else if (settag[current][0] == -2) {
              current_str[current].col(++lastused_tag);
            } else {
              //this junction leg needs to be 'repaired' by having the col and anticolor tags swapped for all partons back to the junction...
              current_str[current].col((settag[current][1] > 0)
                                           ? settag[current][1]
                                           : ++lastused_tag);
              int ptn_now = current;
              std::vector<int> prev_ptns;
              prev_ptns.push_back(ptn_now);
              while (true) {
                int conn_now = 0;
                int ptn_new = -1;
                bool foundnew = false;
                for (int i = 0; i < connections.size(); ++i) {
                  if (connections[ptn_now][i]) {
                    ...
                    for (int j = 0; j < prev_ptns.size(); ++j) {
                      if (i == prev_ptns[j]) {
                        ptn_isprev = true;
                      }
                    }
            ...
            if (connected_all.size() == 1) {
              if (connected_new.size()) {
                settag[connected_new[0]][1] = current_str[current].col();
                if (current_str[connected_new[0]].id() == 21) {
                  settag[connected_new[0]][0] = -1;
                }
              }
            } else {
              for (int i = 0; i < connected_new.size(); ++i) {
                settag[connected_new[i]][0] = -2;
                settag[connected_new[i]][1] = -1;
              }
            }

          } else {
            if (settag[current][1] > 0) {
              current_str[current].acol(settag[current][1]);
            } else if (settag[current][1] == -2) {
              current_str[current].acol(++lastused_tag);
            } else {
              //this junction leg needs to be 'repaired' by having the col and anticolor tags swapped for all partons back to the junction...
              current_str[current].acol((settag[current][0] > 0)
                                            ? settag[current][0]
                                            : ++lastused_tag);
              int ptn_now = current;
              std::vector<int> prev_ptns;
            ...
            if (connected_all.size() == 1) {
              if (connected_new.size()) {
                ...
                if (current_str[connected_new[0]].id() == 21) {
                  settag[connected_new[0]][1] = -1;
                }
              }
            } else {
              for (int i = 0; i < connected_new.size(); ++i) {
                settag[connected_new[i]][0] = -1;
                settag[connected_new[i]][1] = -2;
              }
            }
          }
          if (connected_all.size() > 1) {
            ...
            for (int i = 0; i < connected_all.size(); ++i) {
              temp.push_back(connected_all[i]);
            }
          ...
          if (connected_all.size() == 1) {
            ...
            current_str[0].col(settag[0][0]);
            current_str[0].acol(settag[0][1]);
            current_str[1].col(settag[0][1]);
            current_str[1].acol(settag[0][0]);
            break;
            ...
            if ((connected_new.size() == 2) &&
                (current_str[connected_new[0]].id() == 21) &&
                (current_str[connected_new[1]].id() == 21)) {
              //one of these need to inherit the color, the other, anticolor...
              settag[connected_new[0]][0] = 0;
              settag[connected_new[0]][1] = current_str[current].col();
              settag[connected_new[1]][0] = current_str[current].acol();
              settag[connected_new[1]][1] = -1;
            }
            //this case technically shouldn't happen... but we'll catch and treat, just in case...
              ...
              if ((current_str[connected_new[0]].id() == 21) &&
                ...
                settag[connected_new[0]][0] = current_str[current].acol();
                settag[connected_new[0]][1] = -1;
                settag[connected_new[1]][0] = -1;
                settag[connected_new[1]][1] = current_str[current].col();
              } else if ((std::abs(current_str[connected_new[0]].id()) <= 6) ||
                         ((std::abs(current_str[connected_new[0]].id()) >=
                           1103) &&
                          (std::abs(current_str[connected_new[0]].id()) <=
                           5503) &&
                          ((current_str[connected_new[0]].id() / 10) % 10 ==
                           0))) {
                settag[connected_new[0]][0] = current_str[current].acol();
                settag[connected_new[0]][1] = -1;
                settag[connected_new[1]][0] = -1;
            ...
            std::vector<std::vector<int>> pseudoJunctions;
            std::vector<int> temppartons;
            temppartons.push_back(current);
            pseudoJunctions.push_back(temppartons);
            temppartons.clear();
            ...
            for (int i = 0; i < connected_all.size(); ++i) {
              ...
              for (int j = 0; j < pseudoJunctions.size(); ++j) {
                ...
                for (int k = 0; k < pseudoJunctions[j].size(); ++k) {
                  if (!connections[connected_all[i]][pseudoJunctions[j][k]]) {
                    conn_toall = false;
                  }
                }
                if (conn_toall) {
                  pseudoJunctions[j].push_back(connected_all[i]);
                  added = true;
                }
              }
              if (!added) {
                temppartons.push_back(current);
                temppartons.push_back(connected_all[i]);
                pseudoJunctions.push_back(temppartons);
                temppartons.clear();
              }
            }
            ...
            for (int i = 0; i < pseudoJunctions.size(); ++i) {
              if (pseudoJunctions[i].size() > 2) {
                junctions.push_back(pseudoJunctions[i]);
              }
            }
            ...
            std::vector<int> ptns_inJun;
            std::vector<int> ptns_notJ;
            for (int i = 0; i < connected_new.size(); ++i) {
              bool found_parton = false;
              for (int j = 0; j < pseudoJunctions.size(); ++j) {
                for (int k = 1; k < pseudoJunctions[j].size(); ++k) {
                  if (connected_new[i] == pseudoJunctions[j][k]) {
                    if (pseudoJunctions[j].size() > 2) {
                      ptns_inJun.push_back(connected_new[i]);
                    } else {
                      ptns_notJ.push_back(connected_new[i]);
                    }
                    found_parton = true;
                    break;
                  }
            ...
            for (int i = 0; i < ptns_notJ.size(); ++i) {
              if ((settag[current][0] > 0) && (settag[ptns_notJ[i]][0] < 1)) {
                settag[ptns_notJ[i]][0] = current_str[current].acol();
              } else if ((settag[current][0] == -1) &&
                         (settag[ptns_notJ[i]][0] == 0)) {
                settag[ptns_notJ[i]][0] = -1;
              } else if ((settag[current][0] == -2) &&
                         (settag[ptns_notJ[i]][0] == 0)) {
                settag[ptns_notJ[i]][0] = current_str[current].acol();
              }

              if ((settag[current][1] > 0) && (settag[ptns_notJ[i]][1] < 1)) {
                settag[ptns_notJ[i]][1] = current_str[current].col();
              } else if ((settag[current][1] == -1) &&
                         (settag[ptns_notJ[i]][1] == 0)) {
                settag[ptns_notJ[i]][1] = -1;
              } else if ((settag[current][1] == -2) &&
                         (settag[ptns_notJ[i]][1] == 0)) {
                settag[ptns_notJ[i]][1] = current_str[current].col();
              }
            ...
            for (int i = 0; i < ptns_inJun.size(); ++i) {
              if ((settag[current][0] > 0) && (settag[ptns_inJun[i]][0] < 1)) {
                settag[ptns_inJun[i]][0] = -1;
              } else if ((settag[current][0] == -1) &&
                         (settag[ptns_inJun[i]][0] == 0)) {
                settag[ptns_inJun[i]][0] = -2;
              } else if ((settag[current][0] == -2) &&
                         (settag[ptns_inJun[i]][0] == 0)) {
                settag[ptns_inJun[i]][0] = -1;
              }

              if ((settag[current][1] > 0) && (settag[ptns_inJun[i]][1] < 1)) {
                settag[ptns_inJun[i]][1] = -1;
              } else if ((settag[current][1] == -1) &&
                         (settag[ptns_inJun[i]][1] == 0)) {
                settag[ptns_inJun[i]][1] = -2;
              } else if ((settag[current][1] == -2) &&
                         (settag[ptns_inJun[i]][1] == 0)) {
                settag[ptns_inJun[i]][1] = -1;
              }
    ...
    for (int i = 0; i < junctions.size(); ++i) {
      std::stable_sort(junctions[i].begin(), junctions[i].end());
    }
    ...
    for (int iJun = 0; iJun < junctions.size(); ++iJun) {
      ...
      for (int iPar = 0; iPar < junctions[iJun].size(); ++iPar) {
        if (current_str[junctions[iJun][iPar]].id() != 21) {
          temp.push_back(junctions[iJun][iPar]);
        } else {
          //find the next parton in this leg
          //start by looking for all connected partons
          std::vector<int> connected;
          for (int i = 0; i < connections.size(); ++i) {
            if (connections[junctions[iJun][iPar]][i]) {
          ...
          for (int i = 0; i < connected.size(); ++i) {
            ...
            if ((nconn <= 2 && current_str[connected[i]].id() == 21) ||
                (nconn == 1)) {
              int tempconn = connected[i];
              connected.clear();
              connected.push_back(junctions[iJun][iPar]);
              connected.push_back(tempconn);
              if (current_str[tempconn].id() == 21) {
                findnext = true;
              }
              break;
            } //the below condition only triggers IF we don't find a valid candidate
            else if (i == connected.size() - 1) {
              connected.clear();
              connected.push_back(junctions[iJun][iPar]);
            }
          }
          ...
          if (findnext) {
            int nconn = 0;
            for (int i = 0; i < connections.size(); ++i) {
              if (connections[connected[1]][i]) {
                ++nconn;
              }
            }
            while ((current_str[connected[connected.size() - 1]].id() == 21) &&
                   (nconn <= 2)) {
              for (int i = 0; i < connections.size(); ++i) {
            ...
            if (current_str[connected[connected.size() - 1]].id() != 21) {
              temp.push_back(connected[connected.size() - 1]);
            } else {
              temp.push_back(connected[0]);
            }
          } else {
            if (current_str[connected[connected.size() - 1]].id() != 21) {
              temp.push_back(connected[connected.size() - 1]);
    ...
    parton_collection str_completed;
    std::vector<int> fakequarktype;
    for (int i = 0; i < current_str.num(); ++i) {
      fakequarktype.push_back(0);
    }
    for (int iSib = 0; iSib < py_siblings.size(); ++iSib) {
      int nq = 0;
      int nqbar = 0;
      std::vector<std::vector<int>> gluchains;
      std::vector<int> fakeptns;
      std::vector<int> fakeptns_colid;
      ...
      for (int iPtn = 0; iPtn < py_siblings[iSib].size(); ++iPtn) {
        if (current_str[py_siblings[iSib][iPtn]].id() == 21) {
          ...
          chain.push_back(py_siblings[iSib][iPtn]);
          bool chain_gt_1 = true;
          for (int i = 0; i < connections.size(); ++i) {
            if ((connections[py_siblings[iSib][iPtn]][i]) &&
                (current_str[i].id() == 21)) {
              chain.push_back(i);
            }
          }
          ...
          for (int i = 0; i < chain.size(); ++i) {
            bool issib = false;
            for (int j = 0; j < py_siblings[iSib].size(); ++j) {
              if (chain[i] == py_siblings[iSib][j]) {
                issib = true;
                break;
              }
            }
            ...
            for (int j = 0; j < junctions[iSib].size(); ++j) {
              if (chain[i] == junctions[iSib][j]) {
                issib = true;
                break;
              }
            }
            ...
            for (int j = 0; j < junctions.size(); ++j) {
              ...
              for (int k = 0; k < junctions[j].size(); ++k) {
                if (py_siblings[iSib][iPtn] == junctions[j][k]) {
                  ptn1_inc = true;
                } else if (chain[i] == junctions[j][k]) {
                  ptn2_inc = true;
                }
              }
              if (ptn1_inc && ptn2_inc) {
                issib = true;
                break;
              }
            }
          ...
          if (chain_gt_1) {
            int nconn = 0;
            for (int i = 0; i < connections.size(); ++i) {
              if (connections[chain[1]][i]) {
                ++nconn;
              }
            }
            while (current_str[chain[chain.size() - 1]].id() == 21 &&
                   nconn <= 2) {
              for (int i = 0; i < connections.size(); ++i) {
          ...
          int fakeid = fakequarktype[chain[0]];
          if (fakeid == 0) {
            fakeid = -fakequarktype[chain[chain.size() - 1]];
          }

          if (fakeid == 0) {
            int signJ = 0;
            for (int i = 0; i < py_siblings[iSib].size(); ++i) {
              if ((current_str[py_siblings[iSib][i]].id() == 21) ||
                  (current_str[py_siblings[iSib][i]].id() == 0)) {
                continue;
              } else if ((current_str[py_siblings[iSib][i]].id() > 0 &&
                          current_str[py_siblings[iSib][i]].id() <= 6) ||
                         (current_str[py_siblings[iSib][i]].id() < -6)) {
                ++signJ;
              } else if ((current_str[py_siblings[iSib][i]].id() < 0 &&
                          current_str[py_siblings[iSib][i]].id() >= -6) ||
            ...
            if (signJ >= 0) {
              fakeid = 1;
              fakequarktype[chain[chain.size() - 1]] = -1;
            } else if (signJ < 0) {
              fakeid = -1;
              fakequarktype[chain[chain.size() - 1]] = 1;
            }
          ...
          int col_id = (fakeid == 1) ? current_str[chain[0]].acol()
                                     : current_str[chain[0]].col();
          fakeptns.push_back(fakeid);
          fakeptns_colid.push_back(col_id);

        } else {
          //still need to increment nq, nqbar counters...
          //going to just treat diquarks as antiquarks (and antidiquarks as quarks)
          if ((current_str[py_siblings[iSib][iPtn]].id() > 0 &&
               current_str[py_siblings[iSib][iPtn]].id() <= 6) ||
              (current_str[py_siblings[iSib][iPtn]].id() < -6)) {
            ++nq;
          } else {
            ++nqbar;
      ...
      int ifake = 0;
      int m_qrks[3] = {0, 0, 0};
      bool choosechg = false;
      int chgjun = 0;

      //will fail if this junction has !=3 legs (which it shouldn't be!)
      if (py_siblings[iSib].size() > 3) {
        choosechg = true;
      }
      for (int iPtn = 0; iPtn < py_siblings[iSib].size(); ++iPtn) {
        if (current_str[py_siblings[iSib][iPtn]].id() == 21) {
          m_qrks[iPtn] = fakeptns[ifake];
          ++ifake;
          int chg = (m_qrks[iPtn] % 2 == 0) ? 2 : -1;
          chgjun += chg * (2 * std::signbit(-m_qrks[iPtn]) - 1);
        } else if ((std::abs(current_str[py_siblings[iSib][iPtn]].id()) > 0) &&
                   (std::abs(current_str[py_siblings[iSib][iPtn]].id()) <= 6)) {
          m_qrks[iPtn] = current_str[py_siblings[iSib][iPtn]].id();
          int chg =
              (current_str[py_siblings[iSib][iPtn]].id() % 2 == 0) ? 2 : -1;
          chgjun +=
              chg *
              (2 * std::signbit(-current_str[py_siblings[iSib][iPtn]].id()) -
               1);
        } else if (current_str[py_siblings[iSib][iPtn]].id() < -6) {
          choosechg = true;
          int id1 = (current_str[py_siblings[iSib][iPtn]].id() / 100) % 10;
          chgjun += (id1 % 2 == 0) ? -2 : 1;
          int id2 = (current_str[py_siblings[iSib][iPtn]].id() / 1000) % 10;
          chgjun += (id2 % 2 == 0) ? -2 : 1;
        } else {
          choosechg = true;
          int id1 = (current_str[py_siblings[iSib][iPtn]].id() / 100) % 10;
      ...
      if (!choosechg) {
        if (std::abs(m_qrks[0]) < std::abs(m_qrks[1])) {
          std::swap(m_qrks[0], m_qrks[1]);
        }
        if (std::abs(m_qrks[1]) < std::abs(m_qrks[2])) {
          std::swap(m_qrks[1], m_qrks[2]);
        }
        if (std::abs(m_qrks[0]) < std::abs(m_qrks[1])) {
          std::swap(m_qrks[0], m_qrks[1]);
        }

        if ((m_qrks[0] * m_qrks[1] <= 0) ||
            (m_qrks[0] * m_qrks[2] <= 0) /* || (m_qrks[1]*m_qrks[2]<0)*/) {
          int agree[2] = {0, 0};
          if (m_qrks[0] * m_qrks[1] > 0) {
            agree[0] = m_qrks[0];
            agree[1] = m_qrks[1];
          } else if (m_qrks[0] * m_qrks[2] > 0) {
            agree[0] = m_qrks[0];
            agree[1] = m_qrks[2];
          } else {
            agree[0] = m_qrks[1];
          ...
          m_id = 1000 * m_qrks[0] + 100 * m_qrks[1] + 10 * m_qrks[2] +
                 4; /*if(m_qrks[0]==m_qrks[2]){m_id += 2;}*/
        }
      } else {
        if (chgjun == 6) {
          m_id = 2224;
          ...
          JSWARN << "Junction with noninteger or >2 elecric charge present ("
                 << chgjun << "/3). Unsure of how to properly treat ... "
                 << chgjun;
          m_id = 5554;
        }
      }
      ...
      HHparton mother;
      mother.id(m_id);
      mother.PY_stat(-11); /*mother.PY_stat=-12;*/
      mother.string_id(prev_str);
      mother.orig(-1);
      mother.is_remnant(true);
      mother.mass(1.);
      mother.e(1.);
      if (m_id == 1103) {
        mother.acol(++lastused_tag);
        mother.mass(2. * xmq);
        mother.e(2. * xmq);
      } else if (m_id == -1103) {
        mother.col(++lastused_tag);
        mother.mass(2. * xmq);
        mother.e(2. * xmq);
      } else if (m_id == 1) {
        mother.col(++lastused_tag);
      ...
      for (int iPtn = 0; iPtn < py_siblings[iSib].size(); ++iPtn) {
        if (current_str[py_siblings[iSib][iPtn]].id() != 21) {
          ...
          std::vector<int> jun_leg;
          jun_leg.push_back(py_siblings[iSib][iPtn]);
          int new_conn = 0;
          for (int i = 0; i < connections.size(); ++i) {
          ...
          for (int i = 0; i < jun_leg.size(); ++i) {
            current_str[jun_leg[i]].PY_par1(mother_pos);
          }
          //current_str[py_siblings[iSib][iPtn]].PY_par1(mother_pos);
        } else {
          HHparton fake;
          fake.id(fakeptns[ifake]);
          fake.PY_stat(-21);
          fake.string_id(prev_str);
          fake.orig(-1);
          fake.is_remnant(true);
          fake.PY_par1(mother_pos);
          fake.PY_dau1(-2 - gluchains[ifake][0]);
          fake.PY_dau2(-2 - gluchains[ifake][gluchains[ifake].size() - 1]);
          (fake.id() == 1) ? fake.col(fakeptns_colid[ifake])
                           : fake.acol(fakeptns_colid[ifake]);
          fake.mass(xmq);
          fake.e(xmq);
          str_completed.add(fake);
          for (int ichain = 0; ichain < gluchains[ifake].size(); ++ichain) {
            if (current_str[gluchains[ifake][ichain]].PY_par1() == -1) {
              current_str[gluchains[ifake][ichain]].PY_par1(
                  str_completed.num() - 1);
            } else {
              current_str[gluchains[ifake][ichain]].PY_par2(
                  str_completed.num() - 1);
            }
          }
          ++ifake;
      ...
      if (current_str[py_siblings[iSib][0]].id() != 21) {
        ...
        str_completed[mother_pos].PY_dau1(mother_pos + 1);
      }
      if (current_str[py_siblings[iSib][py_siblings[iSib].size() - 1]].id() !=
          21) {
        str_completed[mother_pos].PY_dau2(
            -2 - py_siblings[iSib][py_siblings[iSib].size() - 1]);
      } else {
        str_completed[mother_pos].PY_dau2(str_completed.num() - 1);
      }
    }

    //reindex appropriate str_completed daughter entries, then add the current_str partons to str_completed
    for (int i = 0; i < str_completed.num(); ++i) {
      if (str_completed[i].PY_dau1() < -1) {
        str_completed[i].PY_dau1(-(2 + str_completed[i].PY_dau1()) +
                                 str_completed.num());
      }
      if (str_completed[i].PY_dau2() < -1) {
        str_completed[i].PY_dau2(-(2 + str_completed[i].PY_dau2()) +
                                 str_completed.num());
      }
    }
    ...
    while (istrfix < str_completed.num()) {
      if (str_completed[istrfix].PY_stat() ==
          -99) { //this is a fake gluon that had to be added - need to remove it and repair any appropriate py_*** entries for other partons
        //since this parton connects two fake partons, we need to repair the color entries for those partons
        str_completed[str_completed[istrfix].PY_par2()].col(
            str_completed[str_completed[istrfix].PY_par1()].acol());
        str_completed[str_completed[istrfix].PY_par2()].acol(
            str_completed[str_completed[istrfix].PY_par1()].col());

        //repairing the py_*** entries for all partons that have values >= istrfix
        for (int i = 0; i < str_completed.num(); ++i) {
          if (str_completed[i].PY_par1() > istrfix) {
            str_completed[i].PY_par1(str_completed[i].PY_par1() - 1);
          } else if (str_completed[i].PY_par1() == istrfix) {
            str_completed[i].PY_par1(-1);
          }
          if (str_completed[i].PY_par2() > istrfix) {
            str_completed[i].PY_par2(str_completed[i].PY_par2() - 1);
          } else if (str_completed[i].PY_par2() == istrfix) {
            str_completed[i].PY_par2(-1);
          }
          if (str_completed[i].PY_dau1() > istrfix) {
            str_completed[i].PY_dau1(str_completed[i].PY_dau1() - 1);
          } else if (str_completed[i].PY_dau1() == istrfix) {
            str_completed[i].PY_dau1(-1);
          }
          if (str_completed[i].PY_dau2() > istrfix) {
            str_completed[i].PY_dau2(str_completed[i].PY_dau2() - 1);
          } else if (str_completed[i].PY_dau2() == istrfix) {
            str_completed[i].PY_dau2(-1);
          }
        }
    ...
    std::stable_sort(
        py_rearrange.begin(), py_rearrange.end(),
    ...
    for (int i = 0; i < py_rearrange.size(); ++i) {
      if (py_rearrange[i][1] != prev_id) {
        if (srt_pos == -1) {
          ...
          int dau2 = -1;
          int dau3 = 99999;
          for (int j = srt_pos; j <= end_pos; ++j) {
            if (py_rearrange[j][2] >= 0) {
              if (dau1 == -1) {
                dau1 = py_rearrange[j][2];
              } else if (dau2 == -1) {
                dau2 = py_rearrange[j][2];
              } else {
                if (py_rearrange[j][2] < dau2) {
                  dau3 = dau2;
                  dau2 = py_rearrange[j][2];
                } else if (py_rearrange[j][2] < dau3) {
                  dau3 = py_rearrange[j][2];
                }
              }
            }
          }
          ...
          for (int j = srt_pos; j <= end_pos; ++j) {
            py_rearrange[j][3] = dau1;
            py_rearrange[j][4] = dau2;
            py_rearrange[j][5] = dau3;
            py_rearrange[j][6] = ncluster;
          }
          end_pos = -1;
          srt_pos = -1;
          prev_id = -1;
          if (py_rearrange[i][1] >= 0) {
            srt_pos = i;
            prev_id = py_rearrange[i][1];
          }
        }
        ...
        [](const std::vector<int> &vec1, const std::vector<int> &vec2) {
          if ((vec1[3] == -1) && (vec2[3] == -1)) {
            ...
            if ((vec1[4] == -1) && (vec2[4] == -1)) {
              ...
              if (vec1[3] == vec2[3]) {
                return false;
              } else if (vec1[4] == vec2[3]) {
                return true;
              } else {
                return vec1[3] < vec2[3];
              }
            } else if ((vec1[4] != -1) && (vec2[4] != -1)) {
              if ((vec1[3] == vec2[3]) || (vec1[3] == vec2[4]) ||
                  (vec1[4] == vec2[3]) || (vec1[4] == vec2[4])) {
                return false;
              } else {
                return vec1[3] < vec2[3];
              }
    ...
    for (int i = 0; i < py_rearrange.size(); ++i) {
      if (py_rearrange[i][1] != prev_id) {
        if (srt_pos == -1) {
          srt_pos = i;
          prev_id = py_rearrange[i][6];
        } else {
          end_pos = i - 1;
          if ((py_rearrange[srt_pos][3] != -1) &&
              (py_rearrange[srt_pos][4] == -1) && first) {
            std::stable_sort(
                py_rearrange.begin() + srt_pos,
                py_rearrange.begin() + end_pos + 1,
                [](const std::vector<int> &vec1, const std::vector<int> &vec2) {
                  return vec1[2] < vec2[2];
                });
            first = false;
          } else if ((py_rearrange[srt_pos][3] != -1) &&
                     (py_rearrange[srt_pos][4] == -1) && !first) {
            std::stable_sort(
                py_rearrange.begin() + srt_pos,
                py_rearrange.begin() + end_pos + 1,
            ...
            std::stable_sort(
                py_rearrange.begin() + srt_pos + 1,
                py_rearrange.begin() + end_pos + 1,
                [](const std::vector<int> &vec1, const std::vector<int> &vec2) {
                  return vec1[2] > vec2[2];
                });
            if (py_rearrange[srt_pos][5] != -1) {
              clusterstofix.push_back(py_rearrange[srt_pos][6]);
            }
          }
          end_pos = -1;
          srt_pos = -1;
          prev_id = -1;
          if (py_rearrange[i][6] >= 0) {
            srt_pos = i;
            prev_id = py_rearrange[i][6];
          }
        }
    ...
    if (srt_pos >= 0) {
      ...
      if ((py_rearrange[srt_pos][3] != -1) &&
          (py_rearrange[srt_pos][4] == -1) && first) {
        std::stable_sort(
            py_rearrange.begin() + srt_pos, py_rearrange.begin() + end_pos + 1,
            [](const std::vector<int> &vec1, const std::vector<int> &vec2) {
              return vec1[2] < vec2[2];
            });
        first = false;
    ...
    while (clusterstofix.size() > 0) {
      ncluster = clusterstofix.back();
      clusterstofix.pop_back();
      //pulling ALL partons in this cluster that need to have a fake generation added
      std::vector<int> ptns_addgen;
      std::vector<int> dau_tofix;
      for (int i = 0; i < py_rearrange.size(); ++i) {
        if (!fixed[i] && (py_rearrange[i][6] == ncluster) &&
            (py_rearrange[i][2] != -1) &&
            (py_rearrange[i][2] != py_rearrange[i][3]) &&
            (py_rearrange[i][2] != py_rearrange[i][4])) {
          ptns_addgen.push_back(i);
          dau_tofix.push_back(py_rearrange[i][2]);
        }
      }
      ...
      for (int i = 0; i < dau_tofix.size(); ++i) {
        ...
        std::vector<int> ptns_fixnow;
        for (int j = 0; j < py_rearrange.size(); ++j) {
          if (py_rearrange[j][2] == dau_tofix[i]) {
            ptns_fixnow.push_back(j);
          }
        }
        ...
        for (int j = 0; j < ptns_fixnow.size(); ++j) {
          ...
          HHparton fakedau = str_completed[py_rearrange[ptns_fixnow[j]][0]];
          fakedau.PY_stat(-21); /*fakedau.PY_stat = -11;*/
          fakedau.PY_par1(py_rearrange[ptns_fixnow[j]][0]);
          fakedau.PY_par2(-1);
          fakedau.PY_dau1(
              str_completed[py_rearrange[ptns_fixnow[j]][0]].PY_dau1());
          fakedau.PY_dau2(
              str_completed[py_rearrange[ptns_fixnow[j]][0]].PY_dau2());
          str_completed.add(fakedau);
          std::vector<int> py_tmp;
          py_tmp.push_back(str_completed.num() - 1);
          py_tmp.push_back(fakedau.PY_par1());
          py_tmp.push_back(fakedau.PY_dau1());
          py_tmp.push_back(-1);
          py_tmp.push_back(-1);
          py_tmp.push_back(-1);
          py_tmp.push_back(0);
          ...
          str_completed[py_rearrange[ptns_fixnow[j]][0]].PY_dau1(
              str_completed.num() - 1);
          str_completed[py_rearrange[ptns_fixnow[j]][0]].PY_dau2(-1);
        }
        ...
        if (str_completed[added_ptns[0]].PY_dau2() > -1) {
          for (int j = 0; j < str_completed.num() - added_ptns.size(); ++j) {
            for (int k = 0; k < added_ptns.size(); ++k) {
              if (str_completed[j].PY_par1() == added_ptns[k]) {
          ...
          str_completed[str_completed[added_ptns[0]].PY_dau1()].PY_par1(
              added_ptns[0]);
          str_completed[str_completed[added_ptns[0]].PY_dau1()].PY_par2(
              added_ptns[added_ptns.size() - 1]);
        }
      }
    }
    ...
    for (int i = 0; i < py_rearrange.size(); ++i) {
      temp_collection.add(str_completed[py_rearrange[i][0]]);
    }
    ...
    for (int i = 0; i < str_completed.num(); ++i) {
      if (str_completed[i].PY_par1() >= 0) {
        str_completed[i].PY_par1(lookup[str_completed[i].PY_par1()]);
      }
      if (str_completed[i].PY_par2() >= 0) {
        str_completed[i].PY_par2(lookup[str_completed[i].PY_par2()]);
      }
    }
      ...
      if (str_completed[i].PY_par1() != prev_moth) {
        if (m_srt == -1) {
          ...
          if (str_completed[m_end].PY_par2() >= 0) {
            ...
            if (m_srt == m_end) {
              str_completed[str_completed[m_end].PY_par2()].PY_dau2(-1);
            }
          }
          m_end = -1;
          m_srt = -1;
    ...
    while (!EP_conserved) {
      EP_conserved = true;
      for (int i = 0; i < str_completed.num(); ++i) {
        if (str_completed[i].PY_stat() >= 0) {
          continue;
        }
        FourVector P_new(0., 0., 0., 0.);
        FourVector pos_new(0., 0., 0., 0.);
        int jmax = (str_completed[i].PY_dau2() > str_completed[i].PY_dau1())
                       ? str_completed[i].PY_dau2()
                       : str_completed[i].PY_dau1();
        for (int j = str_completed[i].PY_dau1(); j < jmax + 1; ++j) {
          double n = double(j - str_completed[i].PY_dau1()) + 1.;
          P_new.Set(P_new.x() + str_completed[j].px(),
                    P_new.y() + str_completed[j].py(),
                    P_new.z() + str_completed[j].pz(),
                    P_new.t() + str_completed[j].e());
          pos_new.Set(pos_new.x() + (str_completed[j].x() - pos_new.x()) / n,
                      pos_new.y() + (str_completed[j].y() - pos_new.y()) / n,
                      pos_new.z() + (str_completed[j].z() - pos_new.z()) / n,
                      pos_new.t() + (str_completed[j].x_t() - pos_new.t()) / n);
        }
        if ((dif2(P_new, str_completed[i].P()) +
                 (P_new.t() - str_completed[i].e()) *
                     (P_new.t() - str_completed[i].e()) >
             0.00000001 /*0.0001^2*/) ||
            (dif2(pos_new, str_completed[i].pos()) +
                 (pos_new.t() - str_completed[i].x_t()) *
                     (pos_new.t() - str_completed[i].x_t()) >
             0.00000001)) {
          str_completed[i].P(P_new);
          str_completed[i].pos(pos_new);
          str_completed[i].mass(str_completed[i].e() * str_completed[i].e() -
                                str_completed[i].px() * str_completed[i].px() -
                                str_completed[i].py() * str_completed[i].py() -
                                str_completed[i].pz() * str_completed[i].pz());
          str_completed[i].mass(
              (str_completed[i].mass() >= 0.)
                  ? sqrt(str_completed[i].mass())
                  : -sqrt(-str_completed[i]
    ...
    for (int i = 0; i < str_completed.num(); ++i) {
      if ((str_completed[i].PY_stat() == -21) &&
          (str_completed[i].PY_dau2() > str_completed[i].PY_dau1())) {
...
bool HybridHadronization::invoke_py() {
  ...
  while (need_hadronization) {
    ...
    const double mm_to_fm = 100000000000.0;
    const double fm_to_mm = 1. / mm_to_fm;
    for (int i = 0; i < HH_pyremn.num(); ++i) {
      //to make sure mass is set appropriately (if E^2-P^2<0, then mass is too)
      double massnow = HH_pyremn[i].e() * HH_pyremn[i].e() -
                       HH_pyremn[i].px() * HH_pyremn[i].px() -
                       HH_pyremn[i].py() * HH_pyremn[i].py() -
                       HH_pyremn[i].pz() * HH_pyremn[i].pz();
      massnow = (massnow >= 0.) ? sqrt(massnow) : -sqrt(-massnow);

      //just in case... (needed for 'low' mass)
      bool recalcE = false;
      double Enow = HH_pyremn[i].e();
      if ((std::abs(HH_pyremn[i].id()) == 1) && (std::abs(massnow) < 0.340)) {
        massnow = (massnow >= 0.) ? 0.340 : -0.340;
        recalcE = true;
      } else if ((std::abs(HH_pyremn[i].id()) == 2) &&
                 (std::abs(massnow) < 0.336)) {
        massnow = (massnow >= 0.) ? 0.336 : -0.336;
        recalcE = true;
      } else if ((std::abs(HH_pyremn[i].id()) == 3) &&
                 (std::abs(massnow) < 0.486)) {
        massnow = (massnow >= 0.) ? 0.486 : -0.486;
        recalcE = true;
      } else if ((std::abs(HH_pyremn[i].id()) == 4) &&
                 (std::abs(massnow) < 1.60)) {
        massnow = (massnow >= 0.) ? 1.60 : -1.60;
        recalcE = true;
      } else if ((std::abs(HH_pyremn[i].id()) == 5) &&
                 (std::abs(massnow) < 4.80)) {
        massnow = (massnow >= 0.) ? 4.80 : -4.80;
        recalcE = true;
      }
      ...
      if (recalcE) {
        Enow = massnow * massnow + HH_pyremn[i].px() * HH_pyremn[i].px() +
               HH_pyremn[i].py() * HH_pyremn[i].py() +
               HH_pyremn[i].pz() * HH_pyremn[i].pz();
      ...
      event.append(HH_pyremn[i].id(), HH_pyremn[i].PY_stat(),
                   ...
                   HH_pyremn[i].py(), HH_pyremn[i].pz(), Enow, massnow);
    ...
    for (int i = 0; i < HH_hadrons.num(); ++i) {
      if (!HH_hadrons[i].is_final()) {
        ...
        event.append(HH_hadrons[i].id(), 81, 0, 0, HH_hadrons[i].px(),
                     HH_hadrons[i].py(), HH_hadrons[i].pz(), HH_hadrons[i].e(),
                     massnow);
        event[event.size() - 1].vProd(
            HH_hadrons[i].x() * fm_to_mm, HH_hadrons[i].y() * fm_to_mm,
            HH_hadrons[i].z() * fm_to_mm, HH_hadrons[i].x_t() * fm_to_mm);
        ++size_input;
        eve_to_had.push_back(i + 1);
      }
    }
    ...
    for (int i = 1; i < event.size(); ++i) {
      if (event[i].isFinal() /*&& (std::abs(event[i].y()) <= 1.)*/) {
        HHhadron
            hadout; //a number of other tags need to be set from the parent (either hadron or string):
        hadout.is_final(true);
        hadout.id(event[i].id());
        hadout.mass(event[i].m());
        hadout.px(event[i].px());
        hadout.py(event[i].py());
        hadout.pz(event[i].pz());
        hadout.e(event[i].e());
        hadout.x(event[i].xProd() * mm_to_fm);
        hadout.y(event[i].yProd() * mm_to_fm);
        hadout.z(event[i].zProd() * mm_to_fm);
        hadout.x_t(event[i].tProd() * mm_to_fm);
        ...
        if ((event[i].mother1() < event[i].mother2()) &&
            (event[i].mother1() > 0) && (std::abs(event[i].status()) >= 81) &&
            (std::abs(event[i].status()) <= 86)) {
          for (int ipar = event[i].mother1(); ipar <= event[i].mother2();
               ++ipar) {
            stack.push_back(ipar);
          }
        } else if ((event[i].mother2() > 0) &&
                   (event[i].mother1() != event[i].mother2())) {
          stack.push_back(event[i].mother1());
          stack.push_back(event[i].mother2());
        } else if (event[i].mother1() > 0) {
          stack.push_back(event[i].mother1());
        } else {
          mothers.push_back(i);
        } //setting it as its own mother if there are no mothers (pythia didn't decay a directly input hadron...)
        ...
        while (stack.size() > 0) {
          ...
          if ((event[current].mother1() < event[current].mother2()) &&
            ...
            for (int ipar = event[current].mother1();
                 ipar <= event[current].mother2(); ++ipar) {
              stack.push_back(ipar);
            }
        ...
        mothers.erase(std::unique(mothers.begin(), mothers.end()),
                      mothers.end());
        ...
        if (mothers[0] <= HH_pyremn.num()) {
          hadout.is_strhad(true);
        } else {
          hadout.is_recohad(true);
        }
        ...
        for (int ipar = 0; ipar < mothers.size(); ++ipar) {
          if (mothers[ipar] <= HH_pyremn.num()) {
            if (HH_pyremn[mothers[ipar] - 1].orig() != -1) {
              hadout.add_par(mothers[ipar] - 1);
              if (HH_pyremn[mothers[ipar] - 1].is_thermal()) {
                is_therm = true;
              }
            }
          } else if (
              mothers[ipar] <=
              size_input) { //shouldn't actually need to check, but doing so just in case.
            hadout.parh(eve_to_had[mothers[ipar]] - 1);
            if (HH_hadrons[hadout.parh()].is_shth()) {
              is_therm = true;
            }
          }
        }
