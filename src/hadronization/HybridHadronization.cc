...
void HybridHadronization::DoHadronization(
  ...
  if (hOut.size() > 0 and reco_hadrons_pythia) {
    for (vector<shared_ptr<Hadron>>::iterator hadIter = hOut.begin();
         hadIter < hOut.end();) {
      ...
      if ((considering[0].id() * considering[1].id() > 0) &&
        ...
        for (int q3 = q2 + 1; q3 < showerquarks.num() + HH_thermal.num();
          ...
          if (WigB[1] * recofactor3 * mult >= rndbaryon) {
            ...
            considering[1].acol() << std::endl; std::cout << considering[2].id()
            << "," << considering[2].col() << "," << considering[2].acol() <<
            std::endl;*/
            if (considering[0].id() * considering[1].id() *
                    considering[2].id() >
                0) {
    ...
    if (reco_hadrons_pythia) {
      ...
      for (int i = 0; i < Initial_hadrons.num(); ++i) {
        ...
        double massnow = Initial_hadrons[i].e() * Initial_hadrons[i].e() -
                         (Initial_hadrons[i].px() * Initial_hadrons[i].px() +
                          Initial_hadrons[i].py() * Initial_hadrons[i].py() +
                          Initial_hadrons[i].pz() * Initial_hadrons[i].pz());
        massnow = (massnow >= 0.) ? sqrt(massnow) : -sqrt(-massnow);
        event.append(Initial_hadrons[i].id(), 81, 0, 0, Initial_hadrons[i].px(),
                     Initial_hadrons[i].py(), Initial_hadrons[i].pz(),
                     Initial_hadrons[i].e(), massnow);
        event[event.size() - 1].vProd(
            Initial_hadrons[i].x(), Initial_hadrons[i].y(),
            Initial_hadrons[i].z(), Initial_hadrons[i].x_t());
        eve_to_had.push_back(i + 1);
      }
