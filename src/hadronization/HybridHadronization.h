...
class HybridHadronization : public HadronizationModule<HybridHadronization> {
 ...
 private:
  ...
  double SigM2_calc(double R2chg, double qm1, double qm2, double qq1,
                    double qq2);
  double SigBR2_calc(double R2chg, double qm1, double qm2, double qm3,
                     double qq1, double qq2, double qq3);
  double SigBL2_calc(double SigBR2, double qm1, double qm2, double qm3);

  // double sigma_pi, sigma_k, sigma_nuc, maxE_level, gmax, xmq, xms, hbarc,
  // dist2cut, sh_recofactor, th_recofactor, SigRB, SigLB;
  double maxM_level, maxB_level, gmax, xmq, xms, xmc, xmb, hbarc, dist2cut,
      sh_recofactor, th_recofactor, p_fake, had_prop, part_prop, delta_t,
      hydro_Tc, eta_max_boost_inv;
  int number_p_fake;
  double SigNucR2, SigNucL2, SigOmgR2, SigOmgL2, SigXiR2, SigXiL2, SigSigR2,
      SigSigL2, SigOcccR2, SigOcccL2, SigOccR2, SigOccL2, SigXiccR2, SigXiccL2,
      SigOcR2, SigOcL2, SigXicR2, SigXicL2, SigSigcR2, SigSigcL2, SigObbbR2,
      SigObbbL2, SigObbcR2, SigObbcL2, SigObbR2, SigObbL2, SigXibbR2, SigXibbL2,
      SigObccR2, SigObccL2, SigObcR2, SigObcL2, SigXibcR2, SigXibcL2, SigObR2,
      SigObL2, SigXibR2, SigXibL2, SigSigbR2, SigSigbL2;
  double SigPi2, SigPhi2, SigK2, SigJpi2, SigDs2, SigD2, SigUps2, SigBc2, SigB2;
  bool inbrick, inhydro;
  int nreusehydro;
  ...
  vector<vector<int>>
      JunctionInfo;  // vector of one junction's color tag and particle info
  ...
  static FourVector HHboost(FourVector B, FourVector vec_in) {
    ...
    double t_out = vec_in.t() * xlam[0][0] + vec_in.x() * xlam[0][1] +
                   vec_in.y() * xlam[0][2] + vec_in.z() * xlam[0][3];
    double x_out = vec_in.t() * xlam[1][0] + vec_in.x() * xlam[1][1] +
                   vec_in.y() * xlam[1][2] + vec_in.z() * xlam[1][3];
    double y_out = vec_in.t() * xlam[2][0] + vec_in.x() * xlam[2][1] +
                   vec_in.y() * xlam[2][2] + vec_in.z() * xlam[2][3];
    double z_out = vec_in.t() * xlam[3][0] + vec_in.x() * xlam[3][1] +
                   vec_in.y() * xlam[3][2] + vec_in.z() * xlam[3][3];
  ...
  class HHparton : public Parton {
   protected:
    // is shower/thermal originating parton; has been used; is a decayed gluon;
    // is a remnant parton; used for reco; used for stringfrag; is endpoint of
    // string
    bool is_shower_, is_thermal_, is_used_, is_decayedglu_, is_remnant_,
        used_reco_, used_str_, is_strendpt_, used_junction_, is_fakep_;
    // id: particle id, ?orig: particle origin(shower, thermal...)?, par: parent
    // parton (perm. set for gluon decays), status: status of particle (is being
    // considered in loop 'i') string_id: string identifier, pos_str: position
    // of parton in string, endpt_id: denotes which endpoint this parton is (of
    // the string), if it is one
    int alt_id_, orig_, par_, string_id_, pos_str_, endpt_id_, sibling_,
        PY_par1_, PY_par2_, PY_dau1_, PY_dau2_, PY_stat_, PY_origid_, PY_tag1_,
        PY_tag2_, PY_tag3_;
    // parton mass
    double alt_mass_;
   ...
   public:
    ...
    HHparton() : Parton::Parton(1, 1, 1, 0., 0., 0., 0.) {
      ...
      is_thermal_ = false;
      is_used_ = false;
      is_decayedglu_ = false;
      is_remnant_ = false;
      used_reco_ = false;
      used_str_ = false;
      is_strendpt_ = false;
      used_junction_ = false;
      is_fakep_ = false;
      alt_id_ = 0;
      orig_ = 0;
      par_ = -1;
    ...
    // getter functions
    double x() { return x_in().x(); }
    double y() { return x_in().y(); }
    double z() { return x_in().z(); }
    double x_t() { return x_in().t(); }
    double px() { return p_in().x(); }
    double py() { return p_in().y(); }
    double pz() { return p_in().z(); }
    double e() { return p_in().t(); }
    bool is_shower() { return is_shower_; }
    bool is_thermal() { return is_thermal_; }
    bool is_used() { return is_used_; }
    bool is_decayedglu() { return is_decayedglu_; }
    bool is_remnant() { return is_remnant_; }
    bool used_reco() { return used_reco_; }
    bool used_str() { return used_str_; }
    bool is_strendpt() { return is_strendpt_; }
    bool used_junction() { return used_junction_; }
    bool is_fakeparton() { return is_fakep_; }
    int id() { return alt_id_; }
    int orig() { return orig_; }
    int par() { return par_; }
    int string_id() const { return string_id_; }
    int pos_str() const { return pos_str_; }
    int endpt_id() { return endpt_id_; }
    int sibling() { return sibling_; }
    int PY_par1() { return PY_par1_; }
    int PY_par2() { return PY_par2_; }
    int PY_dau1() { return PY_dau1_; }
    int PY_dau2() { return PY_dau2_; }
    int PY_stat() { return PY_stat_; }
    int PY_origid() { return PY_origid_; }
    int PY_tag1() { return PY_tag1_; }
    int PY_tag2() { return PY_tag2_; }
    int PY_tag3() { return PY_tag3_; }
    double mass() { return alt_mass_; }
    FourVector pos() { return x_in(); }
    FourVector P() { return p_in(); }
    int status() { return pstat(); }
    int col() { return color(); }
    int acol() { return anti_color(); }
    ...
    void is_shower(bool val) { is_shower_ = val; }
    void is_thermal(bool val) { is_thermal_ = val; }
    void is_used(bool val) { is_used_ = val; }
    void is_decayedglu(bool val) { is_decayedglu_ = val; }
    void is_remnant(bool val) { is_remnant_ = val; }
    void used_reco(bool val) { used_reco_ = val; }
    void used_str(bool val) { used_str_ = val; }
    void is_strendpt(bool val) { is_strendpt_ = val; }
    void used_junction(bool val) { used_junction_ = val; }
    void is_fakeparton(bool val) { is_fakep_ = val; }
    void id(int val) { alt_id_ = val; }
    void orig(int val) { orig_ = val; }
    void par(int val) { par_ = val; }
    void string_id(int val) { string_id_ = val; }
    void pos_str(int val) { pos_str_ = val; }
    void endpt_id(int val) { endpt_id_ = val; }
    void sibling(int val) { sibling_ = val; }
    void PY_par1(int val) { PY_par1_ = val; }
    void PY_par2(int val) { PY_par2_ = val; }
    void PY_dau1(int val) { PY_dau1_ = val; }
    void PY_dau2(int val) { PY_dau2_ = val; }
    void PY_stat(int val) { PY_stat_ = val; }
    void PY_origid(int val) { PY_origid_ = val; }
    void PY_tag1(int val) { PY_tag1_ = val; }
    void PY_tag2(int val) { PY_tag2_ = val; }
    void PY_tag3(int val) { PY_tag3_ = val; }
    void mass(double val) { alt_mass_ = val; }
    void status(int val) { set_stat(val); }
    void col(int val) { set_color(val); }
    void acol(int val) { set_anti_color(val); }

    // boost functions
    FourVector boost_P(FourVector B) { return HHboost(B, p_in()); }
    FourVector boost_P(double vx, double vy, double vz) {
      FourVector B(vx, vy, vz, 0.);
      return HHboost(B, p_in());
    }
    FourVector boost_pos(FourVector B) { return HHboost(B, x_in()); }
    FourVector boost_pos(double vx, double vy, double vz) {
      FourVector B(vx, vy, vz, 0.);
      return HHboost(B, x_in());
    }
  ...
  class HHhadron : public Hadron {
   protected:
    bool is_excited_, is_shsh_, is_shth_, is_thth_, is_recohad_, is_strhad_,
        is_final_;
    // id: particle id, ?orig: particle origin(sh-sh, sh-th, th-th, recombined,
    // stringfragmented...), par_str: parent string number, parh: parent hadron
    // (decayed), status: status of particle (final, used, decayed, etc...)
   ...
   public:
    ...
    double pz() { return p_in().z(); }
    double e() { return p_in().t(); }
    bool is_excited() { return is_excited_; }
    bool is_shsh() { return is_shsh_; }
    bool is_shth() { return is_shth_; }
    bool is_thth() { return is_thth_; }
    bool is_recohad() { return is_recohad_; }
    bool is_strhad() { return is_strhad_; }
    bool is_final() { return is_final_; }
    int orig() { return orig_; }
    int parstr() { return parstr_; }
    int parh() { return parh_; }
    double mass() { return alt_mass_; }
    FourVector pos() { return x_in(); }
    FourVector P() { return p_in(); }
    int id() { return pid(); }
    ...
    void is_excited(bool val) { is_excited_ = val; }
    void is_shsh(bool val) { is_shsh_ = val; }
    void is_shth(bool val) { is_shth_ = val; }
    void is_thth(bool val) { is_thth_ = val; }
    void is_recohad(bool val) { is_recohad_ = val; }
    void is_strhad(bool val) { is_strhad_ = val; }
    void is_final(bool val) { is_final_ = val; }
    void orig(int val) { orig_ = val; }
    void parstr(int val) { parstr_ = val; }
    void parh(int val) { parh_ = val; }
    void mass(double val) { alt_mass_ = val; }
    void id(int val) { set_id(val); }
    void status(int val) { set_stat(val); }
  ...
  class parton_collection {
   public:
    ...
    void add(parton_collection pars) {
      for (int i = 0; i < pars.num(); ++i) {
        partons.push_back(pars[i]);
      }
    }
  ...
  class hadron_collection {
   public:
    ...
    // add a collection of partons to the collection
    void add(hadron_collection hads) {
      for (int i = 0; i < hads.num(); ++i) {
        hadrons.push_back(hads[i]);
      }
    }
  ...
  // functions to set hadron id based on quark content, mass, and if it's in an
  // excited state
  ...
  int findcloserepl(HHparton ptn, int iptn, bool lbt, bool thm,
                    parton_collection& sh_lbt, parton_collection& therm);
  void findcloserepl_glu(HHparton ptn, int iptn, bool lbt, bool thm,
                         parton_collection& sh_lbt, parton_collection& therm,
                         int sel_out[]);

  // function to prepare strings for input into Pythia8
  void stringprep(parton_collection& SP_remnants,
                  parton_collection& SP_prepremn, bool cutstr);

  // function to hand partons/strings and hadron resonances (and other color
  // neutral objects) to Pythia8
  bool invoke_py();

  // function to set the spacetime information for the hadrons coming from
  // pythia
  void set_spacetime_for_pythia_hadrons(Pythia8::Event& event, int& size_input,
                                        std::vector<int>& eve_to_had,
                                        int pythia_attempt, bool find_positions,
                                        bool is_recohadron,
                                        bool recohadron_shsh);

  // function to 'shake' the event a bit to bring the hadrons to their mass
  // shell
  ...
  void convert_color_tags_to_int_type(
      vector<vector<shared_ptr<Parton>>>& shower);

  // scale the energies/momenta of the negative hadrons to have energy
  // conservation
  void scale_kinematics_negative_hadrons(hadron_collection& HH_hadrons,
