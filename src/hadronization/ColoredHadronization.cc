    ...
    for (vector<shared_ptr<Hadron>>::iterator hadIter = hOut.begin();
      ...
      if (hadIter->get()->pid() > 23) {  // skipping leptons
        double massnow = hadIter->get()->e() * hadIter->get()->e() -
                         (hadIter->get()->px() * hadIter->get()->px() +
                          hadIter->get()->py() * hadIter->get()->py() +
                          hadIter->get()->pz() * hadIter->get()->pz());
        massnow = (massnow >= 0.) ? sqrt(massnow) : -sqrt(-massnow);
        event.append(hadIter->get()->pid(), 0, 0, 0, hadIter->get()->px(),
                     hadIter->get()->py(), hadIter->get()->pz(),
                     hadIter->get()->e(), massnow);
