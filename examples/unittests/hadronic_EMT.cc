...
// check coordinate transformation functions (same as in causal_liquifier.cc
// test)
...
TEST(HadronicEMTTest, TEST_LANDAU_MATCHING) {
  ...
  // g^{\mu\nu} e = 1.0, P = e/3, u^{\mu} = (u^0,0.1,0.1,0.1), u^0 = sqrt(1.0 +
  // 0.1^2 + 0.1^2 + 0.1^2) = 1.01488916
  const std::array<std::array<double, 4>, 4> Tmn1 = {
      {{1.04, 0.13531855, 0.13531855, 0.13531855},
...
TEST(HadronicEMTTest, TEST_1D_INTERPOLATION) {
  ...
  const double expected_T =
      pow(hEMT.interpolate_1D_EOS(e_test / hbarC, table), 0.2) * hbarC;
