...
int main(int argc, char** argv) {
  ...
  // commentend out so that you get the dot graph file for the first shower in
  // the first event (add in and the file gets overriden)
  // while (!reader->Finished())
  {
    ...
    for (int i = 0; i < mShowers.size(); i++) {
      ...
      fjcore::ClusterSequence cs(mShowers[i]->GetFinalPartonsForFastJet(),
                                 jet_def);

      vector<fjcore::PseudoJet> jets =
          fjcore::sorted_by_pt(cs.inclusive_jets(2));
      cout << endl;
      ...
      cout << endl;
      cout << "Shower initiating parton : " << *(mShowers[i]->GetPartonAt(0))
           << endl;
      cout << endl;
    ...
    vector<fjcore::PseudoJet> hjets =
        fjcore::sorted_by_pt(hcs.inclusive_jets(2));
