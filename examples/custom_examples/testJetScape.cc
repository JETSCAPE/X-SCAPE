...
int main(int argc, char** argv) {
  ...
  // Think harder and maybe in general/decide on shared vs. unique vs. raw
  // pointer usage ... Current: Just use make_shared always (propably not the
  // most efficient solution ...)

  // auto jetscape =
  // make_shared<JetScape>("/Users/putschke/JetScape/framework_xcode/jetscape_init.xml",3);
  auto jetscape = make_shared<JetScape>();
  ...
  // if << overloaded for classes then for example (see commented out in
  // JetScape.h)
  // cout<<*jetscape<<endl;
  // try to automatically create in Add()!? and to handle things behind the
  // scene ...
  ...
  // maybe smarter to deal with multiple eloss instances (think of di-jet) in
  // seperate task Manager !??
  ...
  REMARK << "since list/vector is sortable, can be used to ensure correct "
            "order or via xml file";

  // --------------------------
  /*
  //cout<<((JetEnergyLoss*) (jloss.get()))->GetQhat()<<endl;
  auto jloss2=make_shared<JetEnergyLoss> (*jloss); //default copy works with STL
  vector list but no deep copy!!? See below
  ...
  cout<<((Matter*) (jloss->GetTaskAt(0).get()))->GetQhat()<<endl; //check with
  shared ... cout<<((Matter*) (jloss2->GetTaskAt(0).get()))->GetQhat()<<endl;
  ...
  */
  ...
  // ...!?) matter->jetSignal.connect(hydro.get(),
  // &FluidDynamics::UpdateEnergyDeposit);
  // matter2->jetSignal.connect(hydro.get(),
  // &FluidDynamics::UpdateEnergyDeposit);
  // martini->jetSignal.connect(hydro.get(),
  // &FluidDynamics::UpdateEnergyDeposit);

  // JSDEBUG<<"Connect Signal/Slot : jetSignal.connect(hydro.get(),
  // &FluidDynamics::UpdateEnergyDeposit);"; REMARK<<"Can be handelded in own
  // connection class ...";
  ...
  //martini.reset();
  //delete martini.get();
  JetScapeLogger::Instance()->Verbose(9)<<" Martini after nullptr ... "<<
  martini.use_count() ;
  ...
  */
  ...
  // Others like finish and write (and in general data structure handling to be
  // added/implmented accordingly)

  // If not handeled via task class can be called individually or put in
  // jetscape main class ...
  // jloss->Init();
  // hydro->Init();
     ...
     for (int i=1;i<20;i++)
      {
        ...
        cout<<i<<" "<<check_fluid_info_ptr->energy_density<<"
  "<<check_fluid_info_ptr->temperature<<endl; cout<<i<<"
  "<<brick_ptr->GetTemperature(i, 1.0, 1.0, 0.0)<<endl; delete
